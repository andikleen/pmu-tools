...
local memory."""
    ...
    def compute(self, EV):
         try:
             self.val = MEM_LOCAL_DRAM_COST * EV("MEM_LOAD_UOPS_LLC_MISS_RETIRED.LOCAL_DRAM", 4) / CLKS(EV, 4)
             self.thresh = (self.val > 0.1) and self.parent.thresh
         except ZeroDivisionError:
             self.val = 0
             self.thresh = False
...
remote memory."""
    ...
    def compute(self, EV):
         try:
             self.val = MEM_REMOTE_DRAM_COST * EV("MEM_LOAD_UOPS_LLC_MISS_RETIRED.REMOTE_DRAM", 4) / CLKS(EV, 4)
             self.thresh = (self.val > 0.1) and self.parent.thresh
         except ZeroDivisionError:
             self.val = 0
             self.thresh = False
...
remote cache in other sockets."""
    ...
    def compute(self, EV):
         try:
             self.val = ( MEM_REMOTE_HITM_COST * EV("MEM_LOAD_UOPS_LLC_MISS_RETIRED.REMOTE_HITM", 4) + MEM_REMOTE_FWD_COST * EV("MEM_LOAD_UOPS_LLC_MISS_RETIRED.REMOTE_FWD", 4) ) / CLKS(EV, 4)
             self.thresh = (self.val > 0.1) and self.parent.thresh
         except ZeroDivisionError:
             self.val = 0
             self.thresh = False
...
from the microcode-sequencer."""
    ...
    def compute(self, EV):
         try:
             ...
             self.thresh = (self.val > 0.7) | self.MicroSequencer.compute(EV)
         except ZeroDivisionError:
             self.val = 0
             self.thresh = False
         return self.val
...
executed."""
    ...
    def compute(self, EV):
         try:
             self.val = 1 - self.FP_Arith.compute(EV) - EV("IDQ.MS_UOPS", 3) / EV("UOPS_EXECUTED.THREAD", 3)
             self.thresh = (self.val > 0.3) and self.parent.thresh
         except ZeroDivisionError:
             self.val = 0
             self.thresh = False
