# -*- coding: latin-1 -*-
#
# auto generated TopDown/TMA 5.1-full description for Intel Core Ultra 200V series (code name Lunar Lake) with Lion Cove
...
def Serialize_Core(self, EV, level):
    return self.Core_Bound.compute(EV) * (self.Serializing_Operation.compute(EV) + EV("RS.EMPTY_RESOURCE", level) / CLKS(self, EV, level) * self.Ports_Utilized_0.compute(EV)) / (self.Serializing_Operation.compute(EV) + self.Ports_Utilization.compute(EV) + self.Divider.compute(EV))
...
def Assist_Frontend(self, EV, level):
    return (1 - EV("INST_RETIRED.REP_ITERATION", level) / EV("UOPS_RETIRED.MS:c1", level)) * (self.Fetch_Latency.compute(EV) * (self.MS_Switches.compute(EV) + self.Branch_Resteers.compute(EV) * (self.Clears_Resteers.compute(EV) + self.Mispredicts_Resteers.compute(EV) * self.Other_Mispredicts.compute(EV) / self.Branch_Mispredicts.compute(EV)) / (self.Mispredicts_Resteers.compute(EV) + self.Clears_Resteers.compute(EV) + self.Unknown_Branches.compute(EV))) / (self.ICache_Misses.compute(EV) + self.ITLB_Misses.compute(EV) + self.LCP.compute(EV) + self.MS_Switches.compute(EV) + self.DSB_Switches.compute(EV) + self.Branch_Resteers.compute(EV)) + self.MS.compute(EV))
...
def Mispredictions(self, EV, level):
    val = 100 *(1 - Umisp(self, EV, level)) * (self.Branch_Mispredicts.compute(EV) + self.Fetch_Latency.compute(EV) * self.Mispredicts_Resteers.compute(EV) / (self.ICache_Misses.compute(EV) + self.ITLB_Misses.compute(EV) + self.LCP.compute(EV) + self.MS_Switches.compute(EV) + self.DSB_Switches.compute(EV) + self.Branch_Resteers.compute(EV)))
    self.thresh = (val > 20)
    return val
...
def Big_Code(self, EV, level):
    val = 100 * self.Fetch_Latency.compute(EV) * (self.ITLB_Misses.compute(EV) + self.ICache_Misses.compute(EV) + self.Unknown_Branches.compute(EV)) / (self.ICache_Misses.compute(EV) + self.ITLB_Misses.compute(EV) + self.LCP.compute(EV) + self.MS_Switches.compute(EV) + self.DSB_Switches.compute(EV) + self.Branch_Resteers.compute(EV))
    self.thresh = (val > 20)
    return val
...
def Instruction_Fetch_BW(self, EV, level):
    val = 100 *(self.Frontend_Bound.compute(EV) - (1 - Umisp(self, EV, level)) * self.Fetch_Latency.compute(EV) * self.Mispredicts_Resteers.compute(EV) / (self.ICache_Misses.compute(EV) + self.ITLB_Misses.compute(EV) + self.LCP.compute(EV) + self.MS_Switches.compute(EV) + self.DSB_Switches.compute(EV) + self.Branch_Resteers.compute(EV)) - Assist_Frontend(self, EV, level)) - Big_Code(self, EV, level)
    self.thresh = (val > 20)
    return val
...
def Data_Cache_Memory_Bandwidth(self, EV, level):
    val = 100 *((self.Memory_Bound.compute(EV) * (self.DRAM_Bound.compute(EV) / (self.L2_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.Store_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.DRAM_Bound.compute(EV))) * (self.MEM_Bandwidth.compute(EV) / (self.MEM_Bandwidth.compute(EV) + self.MEM_Latency.compute(EV)))) + (self.Memory_Bound.compute(EV) * (self.L3_Bound.compute(EV) / (self.L2_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.Store_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.DRAM_Bound.compute(EV))) * (self.SQ_Full.compute(EV) / (self.SQ_Full.compute(EV) + self.Contested_Accesses.compute(EV) + self.L3_Hit_Latency.compute(EV) + self.Data_Sharing.compute(EV)))) + (self.Memory_Bound.compute(EV) * (self.L1_Bound.compute(EV) / (self.L2_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.Store_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.DRAM_Bound.compute(EV))) * (self.FB_Full.compute(EV) / (self.Lock_Latency.compute(EV) + self.Split_Loads.compute(EV) + self.Store_Early_Blk.compute(EV) + self.L1_Latency_Dependency.compute(EV) + self.Store_Fwd_Blk.compute(EV) + self.FB_Full.compute(EV) + self.DTLB_Load.compute(EV) + self.L1_Latency_Capacity.compute(EV)))))
    self.thresh = (val > 20)
    return val

# Total pipeline cost of external Memory- or Cache-Latency related bottlenecks
def Data_Cache_Memory_Latency(self, EV, level):
    val = 100 *((self.Memory_Bound.compute(EV) * (self.DRAM_Bound.compute(EV) / (self.L2_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.Store_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.DRAM_Bound.compute(EV))) * (self.MEM_Latency.compute(EV) / (self.MEM_Bandwidth.compute(EV) + self.MEM_Latency.compute(EV)))) + (self.Memory_Bound.compute(EV) * (self.L3_Bound.compute(EV) / (self.L2_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.Store_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.DRAM_Bound.compute(EV))) * (self.L3_Hit_Latency.compute(EV) / (self.SQ_Full.compute(EV) + self.Contested_Accesses.compute(EV) + self.L3_Hit_Latency.compute(EV) + self.Data_Sharing.compute(EV)))) + (self.Memory_Bound.compute(EV) * self.L2_Bound.compute(EV) / (self.L2_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.Store_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.DRAM_Bound.compute(EV))) + (self.Memory_Bound.compute(EV) * (self.L1_Bound.compute(EV) / (self.L2_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.Store_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.DRAM_Bound.compute(EV))) * (self.L1_Latency_Dependency.compute(EV) / (self.Lock_Latency.compute(EV) + self.Split_Loads.compute(EV) + self.Store_Early_Blk.compute(EV) + self.L1_Latency_Dependency.compute(EV) + self.Store_Fwd_Blk.compute(EV) + self.FB_Full.compute(EV) + self.DTLB_Load.compute(EV) + self.L1_Latency_Capacity.compute(EV)))) + (self.Memory_Bound.compute(EV) * (self.L1_Bound.compute(EV) / (self.L2_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.Store_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.DRAM_Bound.compute(EV))) * (self.L1_Latency_Capacity.compute(EV) / (self.Lock_Latency.compute(EV) + self.Split_Loads.compute(EV) + self.Store_Early_Blk.compute(EV) + self.L1_Latency_Dependency.compute(EV) + self.Store_Fwd_Blk.compute(EV) + self.FB_Full.compute(EV) + self.DTLB_Load.compute(EV) + self.L1_Latency_Capacity.compute(EV)))) + (self.Memory_Bound.compute(EV) * (self.L1_Bound.compute(EV) / (self.L2_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.Store_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.DRAM_Bound.compute(EV))) * (self.Lock_Latency.compute(EV) / (self.Lock_Latency.compute(EV) + self.Split_Loads.compute(EV) + self.Store_Early_Blk.compute(EV) + self.L1_Latency_Dependency.compute(EV) + self.Store_Fwd_Blk.compute(EV) + self.FB_Full.compute(EV) + self.DTLB_Load.compute(EV) + self.L1_Latency_Capacity.compute(EV)))) + (self.Memory_Bound.compute(EV) * (self.L1_Bound.compute(EV) / (self.L2_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.Store_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.DRAM_Bound.compute(EV))) * (self.Split_Loads.compute(EV) / (self.Lock_Latency.compute(EV) + self.Split_Loads.compute(EV) + self.Store_Early_Blk.compute(EV) + self.L1_Latency_Dependency.compute(EV) + self.Store_Fwd_Blk.compute(EV) + self.FB_Full.compute(EV) + self.DTLB_Load.compute(EV) + self.L1_Latency_Capacity.compute(EV)))) + (self.Memory_Bound.compute(EV) * (self.Store_Bound.compute(EV) / (self.L2_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.Store_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.DRAM_Bound.compute(EV))) * (self.Split_Stores.compute(EV) / (self.Store_Latency.compute(EV) + self.DTLB_Store.compute(EV) + self.Streaming_Stores.compute(EV) + self.False_Sharing.compute(EV) + self.Split_Stores.compute(EV)))) + (self.Memory_Bound.compute(EV) * (self.Store_Bound.compute(EV) / (self.L2_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.Store_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.DRAM_Bound.compute(EV))) * (self.Store_Latency.compute(EV) / (self.Store_Latency.compute(EV) + self.DTLB_Store.compute(EV) + self.Streaming_Stores.compute(EV) + self.False_Sharing.compute(EV) + self.Split_Stores.compute(EV)))))
    self.thresh = (val > 20)
    return val

# Total pipeline cost of Memory Address Translation related bottlenecks (data-side TLBs)
def Memory_Data_TLBs(self, EV, level):
    val = 100 *((self.Memory_Bound.compute(EV) * (self.L1_Bound.compute(EV) / (self.L2_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.Store_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.DRAM_Bound.compute(EV))) * (self.DTLB_Load.compute(EV) / (self.Lock_Latency.compute(EV) + self.Split_Loads.compute(EV) + self.Store_Early_Blk.compute(EV) + self.L1_Latency_Dependency.compute(EV) + self.Store_Fwd_Blk.compute(EV) + self.FB_Full.compute(EV) + self.DTLB_Load.compute(EV) + self.L1_Latency_Capacity.compute(EV)))) + (self.Memory_Bound.compute(EV) * (self.Store_Bound.compute(EV) / (self.L2_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.Store_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.DRAM_Bound.compute(EV))) * (self.DTLB_Store.compute(EV) / (self.Store_Latency.compute(EV) + self.DTLB_Store.compute(EV) + self.Streaming_Stores.compute(EV) + self.False_Sharing.compute(EV) + self.Split_Stores.compute(EV)))))
    self.thresh = (val > 20)
    return val
...
def Memory_Synchronization(self, EV, level):
    val = 100 *(self.Memory_Bound.compute(EV) * ((self.L3_Bound.compute(EV) / (self.L2_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.Store_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.DRAM_Bound.compute(EV))) * (self.Contested_Accesses.compute(EV) + self.Data_Sharing.compute(EV)) / (self.SQ_Full.compute(EV) + self.Contested_Accesses.compute(EV) + self.L3_Hit_Latency.compute(EV) + self.Data_Sharing.compute(EV)) + (self.Store_Bound.compute(EV) / (self.L2_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.Store_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.DRAM_Bound.compute(EV))) * self.False_Sharing.compute(EV) / ((self.Store_Latency.compute(EV) + self.DTLB_Store.compute(EV) + self.Streaming_Stores.compute(EV) + self.False_Sharing.compute(EV) + self.Split_Stores.compute(EV)) - self.Store_Latency.compute(EV))) + self.Machine_Clears.compute(EV) * (1 - self.Other_Nukes.compute(EV) / (self.Other_Nukes.compute(EV))))
    self.thresh = (val > 10)
    return val
...
def DSB_Misses(self, EV, level):
    val = 100 *(self.Fetch_Latency.compute(EV) * self.DSB_Switches.compute(EV) / (self.ICache_Misses.compute(EV) + self.ITLB_Misses.compute(EV) + self.LCP.compute(EV) + self.MS_Switches.compute(EV) + self.DSB_Switches.compute(EV) + self.Branch_Resteers.compute(EV)) + self.Fetch_Bandwidth.compute(EV) * self.MITE.compute(EV) / (self.MITE.compute(EV) + self.DSB.compute(EV) + self.LSD.compute(EV) + self.MS.compute(EV)))
    self.thresh = (val > 10)
    return val

# Total pipeline cost of DSB (uop cache) hits - subset of the Instruction_Fetch_BW Bottleneck.
def DSB_Bandwidth(self, EV, level):
    val = 100 *(self.Frontend_Bound.compute(EV) * (self.Fetch_Bandwidth.compute(EV) / (self.Fetch_Latency.compute(EV) + self.Fetch_Bandwidth.compute(EV))) * (self.DSB.compute(EV) / (self.MITE.compute(EV) + self.DSB.compute(EV) + self.LSD.compute(EV) + self.MS.compute(EV))))
    self.thresh = (val > 10)
    return val
...
def IC_Misses(self, EV, level):
    val = 100 *(self.Fetch_Latency.compute(EV) * self.ICache_Misses.compute(EV) / (self.ICache_Misses.compute(EV) + self.ITLB_Misses.compute(EV) + self.LCP.compute(EV) + self.MS_Switches.compute(EV) + self.DSB_Switches.compute(EV) + self.Branch_Resteers.compute(EV)))
    self.thresh = (val > 5)
    return val
...
accesses that hit the L1D cache. The short latency of the
L1D cache may be exposed in pointer-chasing memory access
