# -*- coding: latin-1 -*-
#
# auto generated TopDown/TMA 5.01-full-perf description for Intel Core Ultra 200V series (code name Lunar Lake) with Lion Cove
...
def Serialize_Core(self, EV, level):
    return self.Core_Bound.compute(EV) * (self.Serializing_Operation.compute(EV) + EV("RS.EMPTY_RESOURCE", level) / CLKS(self, EV, level) * self.Ports_Utilized_0.compute(EV)) / (self.Serializing_Operation.compute(EV) + self.Divider.compute(EV) + self.Ports_Utilization.compute(EV))
...
def Mispredictions(self, EV, level):
    val = 100 *(1 - Umisp(self, EV, level)) * (self.Branch_Mispredicts.compute(EV) + self.Fetch_Latency.compute(EV) * self.Mispredicts_Resteers.compute(EV) / (self.LCP.compute(EV) + self.MS_Switches.compute(EV) + self.DSB_Switches.compute(EV) + self.Branch_Resteers.compute(EV) + self.ITLB_Misses.compute(EV) + self.ICache_Misses.compute(EV)))
    self.thresh = (val > 20)
    return val
...
def Big_Code(self, EV, level):
    val = 100 * self.Fetch_Latency.compute(EV) * (self.ITLB_Misses.compute(EV) + self.ICache_Misses.compute(EV) + self.Unknown_Branches.compute(EV)) / (self.LCP.compute(EV) + self.MS_Switches.compute(EV) + self.DSB_Switches.compute(EV) + self.Branch_Resteers.compute(EV) + self.ITLB_Misses.compute(EV) + self.ICache_Misses.compute(EV))
    self.thresh = (val > 20)
    return val
...
def Instruction_Fetch_BW(self, EV, level):
    val = 100 *(self.Frontend_Bound.compute(EV) - (1 - Umisp(self, EV, level)) * self.Fetch_Latency.compute(EV) * self.Mispredicts_Resteers.compute(EV) / (self.LCP.compute(EV) + self.MS_Switches.compute(EV) + self.DSB_Switches.compute(EV) + self.Branch_Resteers.compute(EV) + self.ITLB_Misses.compute(EV) + self.ICache_Misses.compute(EV)) - Assist_Frontend(self, EV, level)) - Big_Code(self, EV, level)
    self.thresh = (val > 20)
    return val

# Total pipeline cost of external Memory- or Cache-Bandwidth related bottlenecks
def Cache_Memory_Bandwidth(self, EV, level):
    val = 100 *((self.Memory_Bound.compute(EV) * (self.DRAM_Bound.compute(EV) / (self.L2_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.Store_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.L3_Bound.compute(EV))) * (self.MEM_Bandwidth.compute(EV) / (self.MEM_Latency.compute(EV) + self.MEM_Bandwidth.compute(EV)))) + (self.Memory_Bound.compute(EV) * (self.L3_Bound.compute(EV) / (self.L2_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.Store_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.L3_Bound.compute(EV))) * (self.SQ_Full.compute(EV) / (self.SQ_Full.compute(EV) + self.Contested_Accesses.compute(EV) + self.Data_Sharing.compute(EV) + self.L3_Hit_Latency.compute(EV)))) + (self.Memory_Bound.compute(EV) * (self.L1_Bound.compute(EV) / (self.L2_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.Store_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.L3_Bound.compute(EV))) * (self.FB_Full.compute(EV) / (self.Lock_Latency.compute(EV) + self.L1_Latency_Capacity.compute(EV) + self.FB_Full.compute(EV) + self.L1_Latency_Dependency.compute(EV) + self.Store_Fwd_Blk.compute(EV) + self.Split_Loads.compute(EV) + self.DTLB_Load.compute(EV)))))
    self.thresh = (val > 20)
    return val

# Total pipeline cost of external Memory- or Cache-Latency related bottlenecks
def Cache_Memory_Latency(self, EV, level):
    val = 100 *((self.Memory_Bound.compute(EV) * (self.DRAM_Bound.compute(EV) / (self.L2_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.Store_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.L3_Bound.compute(EV))) * (self.MEM_Latency.compute(EV) / (self.MEM_Latency.compute(EV) + self.MEM_Bandwidth.compute(EV)))) + (self.Memory_Bound.compute(EV) * (self.L3_Bound.compute(EV) / (self.L2_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.Store_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.L3_Bound.compute(EV))) * (self.L3_Hit_Latency.compute(EV) / (self.SQ_Full.compute(EV) + self.Contested_Accesses.compute(EV) + self.Data_Sharing.compute(EV) + self.L3_Hit_Latency.compute(EV)))) + (self.Memory_Bound.compute(EV) * self.L2_Bound.compute(EV) / (self.L2_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.Store_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.L3_Bound.compute(EV))) + (self.Memory_Bound.compute(EV) * (self.L1_Bound.compute(EV) / (self.L2_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.Store_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.L3_Bound.compute(EV))) * (self.L1_Latency_Dependency.compute(EV) / (self.Lock_Latency.compute(EV) + self.L1_Latency_Capacity.compute(EV) + self.FB_Full.compute(EV) + self.L1_Latency_Dependency.compute(EV) + self.Store_Fwd_Blk.compute(EV) + self.Split_Loads.compute(EV) + self.DTLB_Load.compute(EV)))) + (self.Memory_Bound.compute(EV) * (self.L1_Bound.compute(EV) / (self.L2_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.Store_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.L3_Bound.compute(EV))) * (self.L1_Latency_Capacity.compute(EV) / (self.Lock_Latency.compute(EV) + self.L1_Latency_Capacity.compute(EV) + self.FB_Full.compute(EV) + self.L1_Latency_Dependency.compute(EV) + self.Store_Fwd_Blk.compute(EV) + self.Split_Loads.compute(EV) + self.DTLB_Load.compute(EV)))) + (self.Memory_Bound.compute(EV) * (self.L1_Bound.compute(EV) / (self.L2_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.Store_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.L3_Bound.compute(EV))) * (self.Lock_Latency.compute(EV) / (self.Lock_Latency.compute(EV) + self.L1_Latency_Capacity.compute(EV) + self.FB_Full.compute(EV) + self.L1_Latency_Dependency.compute(EV) + self.Store_Fwd_Blk.compute(EV) + self.Split_Loads.compute(EV) + self.DTLB_Load.compute(EV)))) + (self.Memory_Bound.compute(EV) * (self.L1_Bound.compute(EV) / (self.L2_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.Store_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.L3_Bound.compute(EV))) * (self.Split_Loads.compute(EV) / (self.Lock_Latency.compute(EV) + self.L1_Latency_Capacity.compute(EV) + self.FB_Full.compute(EV) + self.L1_Latency_Dependency.compute(EV) + self.Store_Fwd_Blk.compute(EV) + self.Split_Loads.compute(EV) + self.DTLB_Load.compute(EV)))) + (self.Memory_Bound.compute(EV) * (self.Store_Bound.compute(EV) / (self.L2_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.Store_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.L3_Bound.compute(EV))) * (self.Split_Stores.compute(EV) / (self.Streaming_Stores.compute(EV) + self.False_Sharing.compute(EV) + self.Store_Latency.compute(EV) + self.DTLB_Store.compute(EV) + self.Split_Stores.compute(EV)))) + (self.Memory_Bound.compute(EV) * (self.Store_Bound.compute(EV) / (self.L2_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.Store_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.L3_Bound.compute(EV))) * (self.Store_Latency.compute(EV) / (self.Streaming_Stores.compute(EV) + self.False_Sharing.compute(EV) + self.Store_Latency.compute(EV) + self.DTLB_Store.compute(EV) + self.Split_Stores.compute(EV)))))
    self.thresh = (val > 20)
    return val

# Total pipeline cost of Memory Address Translation related bottlenecks (data-side TLBs)
def Memory_Data_TLBs(self, EV, level):
    val = 100 *((self.Memory_Bound.compute(EV) * (self.L1_Bound.compute(EV) / (self.L2_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.Store_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.L3_Bound.compute(EV))) * (self.DTLB_Load.compute(EV) / (self.Lock_Latency.compute(EV) + self.L1_Latency_Capacity.compute(EV) + self.FB_Full.compute(EV) + self.L1_Latency_Dependency.compute(EV) + self.Store_Fwd_Blk.compute(EV) + self.Split_Loads.compute(EV) + self.DTLB_Load.compute(EV)))) + (self.Memory_Bound.compute(EV) * (self.Store_Bound.compute(EV) / (self.L2_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.Store_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.L3_Bound.compute(EV))) * (self.DTLB_Store.compute(EV) / (self.Streaming_Stores.compute(EV) + self.False_Sharing.compute(EV) + self.Store_Latency.compute(EV) + self.DTLB_Store.compute(EV) + self.Split_Stores.compute(EV)))))
    self.thresh = (val > 20)
    return val
...
def Memory_Synchronization(self, EV, level):
    val = 100 *(self.Memory_Bound.compute(EV) * ((self.L3_Bound.compute(EV) / (self.L2_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.Store_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.L3_Bound.compute(EV))) * (self.Contested_Accesses.compute(EV) + self.Data_Sharing.compute(EV)) / (self.SQ_Full.compute(EV) + self.Contested_Accesses.compute(EV) + self.Data_Sharing.compute(EV) + self.L3_Hit_Latency.compute(EV)) + (self.Store_Bound.compute(EV) / (self.L2_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.Store_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.L3_Bound.compute(EV))) * self.False_Sharing.compute(EV) / ((self.Streaming_Stores.compute(EV) + self.False_Sharing.compute(EV) + self.Store_Latency.compute(EV) + self.DTLB_Store.compute(EV) + self.Split_Stores.compute(EV)) - self.Store_Latency.compute(EV))) + self.Machine_Clears.compute(EV) * (1 - self.Other_Nukes.compute(EV) / (self.Other_Nukes.compute(EV))))
    self.thresh = (val > 10)
    return val
...
def Compute_Bound_Est(self, EV, level):
    val = 100 *((self.Core_Bound.compute(EV) * self.Divider.compute(EV) / (self.Serializing_Operation.compute(EV) + self.Divider.compute(EV) + self.Ports_Utilization.compute(EV))) + (self.Core_Bound.compute(EV) * (self.Ports_Utilization.compute(EV) / (self.Serializing_Operation.compute(EV) + self.Divider.compute(EV) + self.Ports_Utilization.compute(EV))) * (self.Ports_Utilized_3m.compute(EV) / (self.Ports_Utilized_2.compute(EV) + self.Ports_Utilized_0.compute(EV) + self.Ports_Utilized_3m.compute(EV) + self.Ports_Utilized_1.compute(EV)))))
    self.thresh = (val > 20)
    return val
...
def DSB_Misses(self, EV, level):
    val = 100 *(self.Fetch_Latency.compute(EV) * self.DSB_Switches.compute(EV) / (self.LCP.compute(EV) + self.MS_Switches.compute(EV) + self.DSB_Switches.compute(EV) + self.Branch_Resteers.compute(EV) + self.ITLB_Misses.compute(EV) + self.ICache_Misses.compute(EV)) + self.Fetch_Bandwidth.compute(EV) * self.MITE.compute(EV) / (self.LSD.compute(EV) + self.MITE.compute(EV) + self.MS.compute(EV) + self.DSB.compute(EV)))
    self.thresh = (val > 10)
    return val

# Total pipeline cost of DSB (uop cache) hits - subset of the Instruction_Fetch_BW Bottleneck.
def DSB_Bandwidth(self, EV, level):
    val = 100 *(self.Frontend_Bound.compute(EV) * (self.Fetch_Bandwidth.compute(EV) / (self.Fetch_Latency.compute(EV) + self.Fetch_Bandwidth.compute(EV))) * (self.DSB.compute(EV) / (self.LSD.compute(EV) + self.MITE.compute(EV) + self.MS.compute(EV) + self.DSB.compute(EV))))
    self.thresh = (val > 10)
    return val
...
def IC_Misses(self, EV, level):
    val = 100 *(self.Fetch_Latency.compute(EV) * self.ICache_Misses.compute(EV) / (self.LCP.compute(EV) + self.MS_Switches.compute(EV) + self.DSB_Switches.compute(EV) + self.Branch_Resteers.compute(EV) + self.ITLB_Misses.compute(EV) + self.ICache_Misses.compute(EV)))
    self.thresh = (val > 5)
    return val
...
def DRAM_BW_Use(self, EV, level):
    return 64 * EV("UNC_ARB_TRK_REQUESTS.ALL", level) / OneMillion / Time(self, EV, level) / 1000
...
class DSB:
    ...
    def compute(self, EV):
        try:
            self.val = (EV("IDQ.DSB_UOPS:c8:i1:eq1", 3) + EV("IDQ.DSB_UOPS", 3) / (EV("IDQ.DSB_UOPS", 3) + EV("IDQ.MITE_UOPS", 3)) * (EV("IDQ_BUBBLES.CYCLES_0_UOPS_DELIV.CORE", 3) - EV("IDQ_BUBBLES.FETCH_LATENCY", 3))) / CLKS(self, EV, 3)
...
class LSD:
    ...
    def compute(self, EV):
        try:
            self.val = EV("LSD.UOPS:c8:i1:eq1", 3) / CLKS(self, EV, 3)
            self.thresh = (self.val > 0.15) and self.parent.thresh
        except ZeroDivisionError:
...
class MS:
    ...
    def compute(self, EV):
        try:
            self.val = EV("IDQ.MS_CYCLES_ANY", 3) / CLKS(self, EV, 3)
            self.thresh = (self.val > 0.05) and self.parent.thresh
        except ZeroDivisionError:
...
class L1_Latency_Dependency:
    ...
    def compute(self, EV):
        try:
            self.val = 4 * EV("DEPENDENT_LOADS.ANY", 4) / CLKS(self, EV, 4)
            self.thresh = (self.val > 0.1) and self.parent.thresh
        except ZeroDivisionError:
...
class False_Sharing:
    ...
    def compute(self, EV):
        try:
            self.val = 0.0 # XXX w/a for broken kernel
...
class Streaming_Stores:
    ...
    def compute(self, EV):
        try:
            self.val = 0.0 # W/A for broken kernel
...
class FP_Vector_256b:
    ...
    def compute(self, EV):
        try:
            self.val = EV("FP_ARITH_INST_RETIRED.VECTOR:u0x30", 5) / Retired_Slots(self, EV, 5)
            self.thresh = (self.val > 0.1) and self.parent.thresh
        except ZeroDivisionError:
...
class Setup:
    def __init__(self, r):
        ...
        o["Cache_Memory_Bandwidth"].L1_Latency_Capacity = o["L1_Latency_Capacity"]
        ...
        o["Cache_Memory_Latency"].L1_Latency_Capacity = o["L1_Latency_Capacity"]
