# -*- coding: latin-1 -*-
#
# auto generated TopDown/TMA 5.2-full description for Intel Panther Lake with Cougar Cove
...
def Br_DoI_Jumps(self, EV, level):
    return EV("BR_INST_RETIRED.NEAR_TAKEN", level) - EV("BR_INST_RETIRED.COND_TAKEN_BWD", level) - EV("BR_INST_RETIRED.COND_TAKEN_FWD", level) - 2 * EV("BR_INST_RETIRED.NEAR_CALL", level)
...
def Serialize_Core(self, EV, level):
    return self.Core_Bound.compute(EV) * (self.Serializing_Operation.compute(EV) + EV("RS.EMPTY_RESOURCE", level) / CLKS(self, EV, level) * self.Ports_Utilized_0.compute(EV)) / (self.Ports_Utilization.compute(EV) + self.Serializing_Operation.compute(EV) + self.Divider.compute(EV))
...
def Assist(self, EV, level):
    return (self.Microcode_Sequencer.compute(EV) / (self.Microcode_Sequencer.compute(EV) + max(0 , self.Heavy_Operations.compute(EV) - self.Microcode_Sequencer.compute(EV)))) * (Avg_Assist_Cost * EV("ASSISTS.ANY", level) / SLOTS(self, EV, level) / self.Microcode_Sequencer.compute(EV))

def Assist_Frontend(self, EV, level):
    return (1 - EV("INST_RETIRED.REP_ITERATION", level) / EV("UOPS_RETIRED.MS:c1", level)) * (self.Fetch_Latency.compute(EV) * (self.MS_Switches.compute(EV) + self.Branch_Resteers.compute(EV) * (self.Clears_Resteers.compute(EV) + self.Mispredicts_Resteers.compute(EV) * self.Other_Mispredicts.compute(EV) / self.Branch_Mispredicts.compute(EV)) / (self.Mispredicts_Resteers.compute(EV) + self.Clears_Resteers.compute(EV) + self.Unknown_Branches.compute(EV))) / (self.ITLB_Misses.compute(EV) + self.MS_Switches.compute(EV) + self.LCP.compute(EV) + self.DSB_Switches.compute(EV) + self.Branch_Resteers.compute(EV) + self.ICache_Misses.compute(EV)) + self.MS.compute(EV))
...
def FLOP_Count(self, EV, level):
    return EV("FP_ARITH_OPS_RETIRED.SCALAR", level) + 2 * EV("FP_ARITH_OPS_RETIRED.128B_PACKED_DOUBLE", level) + 4 * EV("FP_ARITH_OPS_RETIRED.4_FLOPS", level) + 8 * EV("FP_ARITH_OPS_RETIRED.256B_PACKED_SINGLE", level)

def FP_Arith_Scalar(self, EV, level):
    return EV("FP_ARITH_OPS_RETIRED.SCALAR", level)

def FP_Arith_Vector(self, EV, level):
    return EV("FP_ARITH_OPS_RETIRED.VECTOR", level)
...
def Light_Ops_Sum(self, EV, level):
    return self.Retiring.compute(EV) * EV("UOPS_EXECUTED.X87", level) / EV("UOPS_EXECUTED.THREAD", level) + (FP_Arith_Scalar(self, EV, level) + FP_Arith_Vector(self, EV, level)) / Retired_Slots(self, EV, level) + (EV("INT_VEC_RETIRED.ADD_128", level) + EV("INT_VEC_RETIRED.VNNI_128", level) + EV("INT_VEC_RETIRED.ADD_256", level) + EV("INT_VEC_RETIRED.MUL_256", level) + EV("INT_VEC_RETIRED.VNNI_256", level)) / Retired_Slots(self, EV, level) + self.Memory_Operations.compute(EV) + self.Fused_Instructions.compute(EV) + self.Non_Fused_Branches.compute(EV)

def MEM_Bound_Ratio(self, EV, level):
    return EV("MEMORY_STALLS.MEM", level) / CLKS(self, EV, level)
...
def Num_CPUs(self, EV, level):
    return num_cores if num_cores else 16

# Total pipeline cost of Branch Misprediction related bottlenecks
def Mispredictions(self, EV, level):
    val = 100 *(1 - Umisp(self, EV, level)) * (self.Branch_Mispredicts.compute(EV) + self.Fetch_Latency.compute(EV) * self.Mispredicts_Resteers.compute(EV) / (self.ITLB_Misses.compute(EV) + self.MS_Switches.compute(EV) + self.LCP.compute(EV) + self.DSB_Switches.compute(EV) + self.Branch_Resteers.compute(EV) + self.ICache_Misses.compute(EV)))
    self.thresh = (val > 20)
    return val
...
def Big_Code(self, EV, level):
    val = 100 * self.Fetch_Latency.compute(EV) * (self.ITLB_Misses.compute(EV) + self.ICache_Misses.compute(EV) + self.Unknown_Branches.compute(EV)) / (self.ITLB_Misses.compute(EV) + self.MS_Switches.compute(EV) + self.LCP.compute(EV) + self.DSB_Switches.compute(EV) + self.Branch_Resteers.compute(EV) + self.ICache_Misses.compute(EV))
    self.thresh = (val > 20)
    return val
...
def Instruction_Fetch_BW(self, EV, level):
    val = 100 *(self.Frontend_Bound.compute(EV) - (1 - Umisp(self, EV, level)) * self.Fetch_Latency.compute(EV) * self.Mispredicts_Resteers.compute(EV) / (self.ITLB_Misses.compute(EV) + self.MS_Switches.compute(EV) + self.LCP.compute(EV) + self.DSB_Switches.compute(EV) + self.Branch_Resteers.compute(EV) + self.ICache_Misses.compute(EV)) - Assist_Frontend(self, EV, level)) - Big_Code(self, EV, level)
    self.thresh = (val > 20)
    return val
...
def Data_Cache_Memory_Bandwidth(self, EV, level):
    val = 100 *((self.Memory_Bound.compute(EV) * (self.L3_Miss_Bound.compute(EV) / (self.L1_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.Store_Bound.compute(EV) + self.L3_Miss_Bound.compute(EV) + self.L2_Bound.compute(EV))) * (self.MEM_Bandwidth.compute(EV) / (self.UC_Bound.compute(EV) + self.MEM_Bandwidth.compute(EV) + self.MEM_Latency.compute(EV)))) + (self.Memory_Bound.compute(EV) * (self.L3_Bound.compute(EV) / (self.L1_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.Store_Bound.compute(EV) + self.L3_Miss_Bound.compute(EV) + self.L2_Bound.compute(EV))) * (self.SQ_Full.compute(EV) / (self.Data_Sharing.compute(EV) + self.L3_Hit_Latency.compute(EV) + self.SQ_Full.compute(EV) + self.Contested_Accesses.compute(EV)))) + (self.Memory_Bound.compute(EV) * (self.L1_Bound.compute(EV) / (self.L1_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.Store_Bound.compute(EV) + self.L3_Miss_Bound.compute(EV) + self.L2_Bound.compute(EV))) * (self.FB_Full.compute(EV) / (self.Store_Early_Blk.compute(EV) + self.FB_Full.compute(EV) + self.Store_Fwd_Blk.compute(EV) + self.DTLB_Load.compute(EV) + self.Lock_Latency.compute(EV) + self.Split_Loads.compute(EV) + self.L1_Latency_Capacity.compute(EV)))))
    self.thresh = (val > 20)
    return val

# Total pipeline cost of external Memory- or Cache-Latency related bottlenecks
def Data_Cache_Memory_Latency(self, EV, level):
    val = 100 * self.Memory_Bound.compute(EV) * ((self.L3_Miss_Bound.compute(EV) / (self.L1_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.Store_Bound.compute(EV) + self.L3_Miss_Bound.compute(EV) + self.L2_Bound.compute(EV))) * (self.MEM_Latency.compute(EV) / (self.UC_Bound.compute(EV) + self.MEM_Bandwidth.compute(EV) + self.MEM_Latency.compute(EV))) + (self.L3_Bound.compute(EV) / (self.L1_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.Store_Bound.compute(EV) + self.L3_Miss_Bound.compute(EV) + self.L2_Bound.compute(EV))) * (self.L3_Hit_Latency.compute(EV) / (self.Data_Sharing.compute(EV) + self.L3_Hit_Latency.compute(EV) + self.SQ_Full.compute(EV) + self.Contested_Accesses.compute(EV))) + (self.L2_Bound.compute(EV) / (self.L1_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.Store_Bound.compute(EV) + self.L3_Miss_Bound.compute(EV) + self.L2_Bound.compute(EV))) + (self.L1_Bound.compute(EV) / (self.L1_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.Store_Bound.compute(EV) + self.L3_Miss_Bound.compute(EV) + self.L2_Bound.compute(EV))) * (self.L1_Latency_Capacity.compute(EV) / (self.Store_Early_Blk.compute(EV) + self.FB_Full.compute(EV) + self.Store_Fwd_Blk.compute(EV) + self.DTLB_Load.compute(EV) + self.Lock_Latency.compute(EV) + self.Split_Loads.compute(EV) + self.L1_Latency_Capacity.compute(EV))))
    self.thresh = (val > 20)
    return val

# Total pipeline cost of Memory Address Translation related bottlenecks (data-side TLBs)
def Memory_Data_TLBs(self, EV, level):
    val = 100 *((self.Memory_Bound.compute(EV) * (self.L1_Bound.compute(EV) / (self.L1_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.Store_Bound.compute(EV) + self.L3_Miss_Bound.compute(EV) + self.L2_Bound.compute(EV))) * (self.DTLB_Load.compute(EV) / (self.Store_Early_Blk.compute(EV) + self.FB_Full.compute(EV) + self.Store_Fwd_Blk.compute(EV) + self.DTLB_Load.compute(EV) + self.Lock_Latency.compute(EV) + self.Split_Loads.compute(EV) + self.L1_Latency_Capacity.compute(EV)))) + (self.Memory_Bound.compute(EV) * (self.Store_Bound.compute(EV) / (self.L1_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.Store_Bound.compute(EV) + self.L3_Miss_Bound.compute(EV) + self.L2_Bound.compute(EV))) * (self.DTLB_Store.compute(EV) / (self.Store_Latency.compute(EV) + self.Streaming_Stores.compute(EV) + self.False_Sharing.compute(EV) + self.Split_Stores.compute(EV) + self.DTLB_Store.compute(EV)))))
    self.thresh = (val > 20)
    return val
...
def Memory_Synchronization(self, EV, level):
    val = 100 *(self.Memory_Bound.compute(EV) * ((self.L3_Bound.compute(EV) / (self.L1_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.Store_Bound.compute(EV) + self.L3_Miss_Bound.compute(EV) + self.L2_Bound.compute(EV))) * (self.Contested_Accesses.compute(EV) + self.Data_Sharing.compute(EV)) / (self.Data_Sharing.compute(EV) + self.L3_Hit_Latency.compute(EV) + self.SQ_Full.compute(EV) + self.Contested_Accesses.compute(EV)) + (self.Store_Bound.compute(EV) / (self.L1_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.Store_Bound.compute(EV) + self.L3_Miss_Bound.compute(EV) + self.L2_Bound.compute(EV))) * self.False_Sharing.compute(EV) / ((self.Store_Latency.compute(EV) + self.Streaming_Stores.compute(EV) + self.False_Sharing.compute(EV) + self.Split_Stores.compute(EV) + self.DTLB_Store.compute(EV)) - self.Store_Latency.compute(EV))) + self.Machine_Clears.compute(EV) * (1 - self.Other_Nukes.compute(EV) / (self.Other_Nukes.compute(EV))))
    self.thresh = (val > 10)
    return val
...
def Compute_Bound_Est(self, EV, level):
    val = 100 *((self.Core_Bound.compute(EV) * self.Divider.compute(EV) / (self.Ports_Utilization.compute(EV) + self.Serializing_Operation.compute(EV) + self.Divider.compute(EV))) + (self.Core_Bound.compute(EV) * (self.Ports_Utilization.compute(EV) / (self.Ports_Utilization.compute(EV) + self.Serializing_Operation.compute(EV) + self.Divider.compute(EV))) * (self.Ports_Utilized_3m.compute(EV) / (self.Ports_Utilized_3m.compute(EV) + self.Ports_Utilized_1.compute(EV) + self.Ports_Utilized_2.compute(EV) + self.Ports_Utilized_0.compute(EV)))))
    self.thresh = (val > 20)
    return val
...
def SLOTS(self, EV, level):
    return EV("TOPDOWN.SLOTS", level) if topdown_use_fixed else EV("TOPDOWN.SLOTS_P", level)
...
def FLOPc(self, EV, level):
    return FLOP_Count(self, EV, level) / CLKS(self, EV, level)
...
def FP_Arith_Utilization(self, EV, level):
    return (EV("FP_ARITH_DISPATCHED.V0", level) + EV("FP_ARITH_DISPATCHED.V1", level) + EV("FP_ARITH_DISPATCHED.V2", level) + EV("FP_ARITH_DISPATCHED.V3", level)) / (4 * CLKS(self, EV, level))
...
def IpArith_Scalar_SP(self, EV, level):
    val = EV("INST_RETIRED.ANY", level) / EV("FP_ARITH_OPS_RETIRED.SCALAR_SINGLE", level)
    self.thresh = (val < 10)
    return val
...
def IpArith_Scalar_DP(self, EV, level):
    val = EV("INST_RETIRED.ANY", level) / EV("FP_ARITH_OPS_RETIRED.SCALAR_DOUBLE", level)
    self.thresh = (val < 10)
    return val
...
def IpArith_AVX128(self, EV, level):
    val = EV("INST_RETIRED.ANY", level) / (EV("FP_ARITH_OPS_RETIRED.128B_PACKED_DOUBLE", level) + EV("FP_ARITH_OPS_RETIRED.128B_PACKED_SINGLE", level))
    self.thresh = (val < 10)
    return val
...
def IpArith_AVX256(self, EV, level):
    val = EV("INST_RETIRED.ANY", level) / (EV("FP_ARITH_OPS_RETIRED.256B_PACKED_DOUBLE", level) + EV("FP_ARITH_OPS_RETIRED.256B_PACKED_SINGLE", level))
    self.thresh = (val < 10)
    return val

# AVX preserve/restore assists per kilo instruction
def VectorMixPKI(self, EV, level):
    val = 1000 * EV("ASSISTS.SSE_AVX_MIX", level) / EV("INST_RETIRED.ANY", level)
...
def IpSWPF(self, EV, level):
    val = EV("INST_RETIRED.ANY", level) / EV("MEM_INST_RETIRED.ALL_SWPF", level)
    self.thresh = (val < 100)
    return val
...
# Total percent of cycles without actually retired uops
def Retire_Stall(self, EV, level):
    return EV("UOPS_RETIRED.STALLS", level) / CLKS(self, EV, level)
...
# Average number of uops fetched from MS per cycle
def Fetch_MS(self, EV, level):
    return EV("IDQ.MS_UOPS", level) / EV("IDQ.MS_UOPS:c1", level)
...
def DSB_Misses(self, EV, level):
    val = 100 *(self.Fetch_Latency.compute(EV) * self.DSB_Switches.compute(EV) / (self.ITLB_Misses.compute(EV) + self.MS_Switches.compute(EV) + self.LCP.compute(EV) + self.DSB_Switches.compute(EV) + self.Branch_Resteers.compute(EV) + self.ICache_Misses.compute(EV)) + self.Fetch_Bandwidth.compute(EV) * self.MITE.compute(EV) / (self.MITE.compute(EV) + self.DSB.compute(EV) + self.MS.compute(EV)))
    self.thresh = (val > 10)
    return val

# Total pipeline cost of DSB (uop cache) hits - subset of the Instruction_Fetch_BW Bottleneck.
def DSB_Bandwidth(self, EV, level):
    val = 100 *(self.Frontend_Bound.compute(EV) * (self.Fetch_Bandwidth.compute(EV) / (self.Fetch_Bandwidth.compute(EV) + self.Fetch_Latency.compute(EV))) * (self.DSB.compute(EV) / (self.MITE.compute(EV) + self.DSB.compute(EV) + self.MS.compute(EV))))
    self.thresh = (val > 10)
    return val
...
def IC_Misses(self, EV, level):
    val = 100 *(self.Fetch_Latency.compute(EV) * self.ICache_Misses.compute(EV) / (self.ITLB_Misses.compute(EV) + self.MS_Switches.compute(EV) + self.LCP.compute(EV) + self.DSB_Switches.compute(EV) + self.Branch_Resteers.compute(EV) + self.ICache_Misses.compute(EV)))
    self.thresh = (val > 5)
    return val
...
# Instructions per retired Mispredicts for conditional backward-taken branches (lower number means higher occurrence rate).
def IpMisp_Cond_Taken_Bwd(self, EV, level):
    return EV("INST_RETIRED.ANY", level) / EV("BR_MISP_RETIRED.COND_TAKEN_BWD", level)

# Instructions per retired Mispredicts for conditional forward-taken branches (lower number means higher occurrence rate).
def IpMisp_Cond_Taken_Fwd(self, EV, level):
    return EV("INST_RETIRED.ANY", level) / EV("BR_MISP_RETIRED.COND_TAKEN_FWD", level)
...
# Fraction of branches that are forward taken conditionals
def Cond_TK_Bwd(self, EV, level):
    val = EV("BR_INST_RETIRED.COND_TAKEN_BWD", level) / EV("BR_INST_RETIRED.ALL_BRANCHES", level)
    self.thresh = (val > 0.3)
    return val

# Fraction of branches that are forward taken conditionals
def Cond_TK_Fwd(self, EV, level):
    val = EV("BR_INST_RETIRED.COND_TAKEN_FWD", level) / EV("BR_INST_RETIRED.ALL_BRANCHES", level)
    self.thresh = (val > 0.2)
    return val
...
def Other_Branches(self, EV, level):
    return 1 -(Cond_NT(self, EV, level) + Cond_TK_Bwd(self, EV, level) + Cond_TK_Fwd(self, EV, level) + CallRet(self, EV, level) + Jump(self, EV, level))
...
def Load_Miss_Real_Latency(self, EV, level):
    return EV("L1D_PENDING.LOAD", level) / EV("L1D_MISS.LOAD", level)
...
def MLP(self, EV, level):
    return EV("L1D_PENDING.LOAD", level) / EV("L1D_PENDING.LOAD_CYCLES", level)

# L0 cache true misses per kilo instruction for retired demand loads
def L1DL0_MPKI(self, EV, level):
    return 1000 *(EV("MEM_LOAD_RETIRED.L1_MISS", level) + EV("MEM_LOAD_RETIRED.L1_HIT_L1", level)) / EV("INST_RETIRED.ANY", level)
...
# Average per-thread data fill bandwidth to the Level 0 within L1D cache [GB / sec]
def L1DL0_Cache_Fill_BW(self, EV, level):
    return 64 * EV("L1D.L0_REPLACEMENT", level) / OneBillion / Time(self, EV, level)

# Average per-thread data fill bandwidth to the L1 data cache [GB / sec]
def L1D_Cache_Fill_BW(self, EV, level):
    return 64 * EV("L1D.L1_REPLACEMENT", level) / OneBillion / Time(self, EV, level)
...
def Page_Walks_Utilization(self, EV, level):
    val = (EV("ITLB_MISSES.WALK_PENDING", level) + EV("DTLB_LOAD_MISSES.WALK_PENDING", level) + EV("DTLB_STORE_MISSES.WALK_PENDING", level)) / (4 * CLKS(self, EV, level))
    self.thresh = (val > 0.5)
    return val
...
def DRAM_BW_Use(self, EV, level):
    return 32 * EV("UNC_M_TOTAL_DATA", level) / OneBillion / Time(self, EV, level)
...
class Frontend_Bound:
    ...
    def compute(self, EV):
        try:
            self.val = (EV("PERF_METRICS.FRONTEND_BOUND", 1) / EV("TOPDOWN.SLOTS", 1)) / PERF_METRICS_SUM(self, EV, 1) if topdown_use_fixed else EV("IDQ_BUBBLES.CORE", 1) / SLOTS(self, EV, 1)
...
class Fetch_Latency:
    ...
    def compute(self, EV):
        try:
            self.val = (EV("PERF_METRICS.FETCH_LATENCY", 2) / EV("TOPDOWN.SLOTS", 2)) / PERF_METRICS_SUM(self, EV, 2) if topdown_use_fixed else EV("IDQ_BUBBLES.FETCH_LATENCY", 2) / CLKS(self, EV, 2)
...
class ICache_Misses:
    ...
    def compute(self, EV):
        try:
            self.val = EV("ICACHE_DATA.STALLS", 3) / CLKS(self, EV, 3) + self.ISB_Full.compute(EV)
...
class ISB_Full:
    ...
    def compute(self, EV):
        try:
            self.val = (EV("ICACHE_TAG.STALLS_ISB", 4) + EV("ICACHE_TAG.STALLS_INUSE", 4)) / CLKS(self, EV, 4)
            self.thresh = (self.val > 0.05) and self.parent.thresh
        except ZeroDivisionError:
            handle_error(self, "ISB_Full zero division")
        return self.val
    desc = """
This metric represents fraction of cycles Icache tag lookup
was stalled due to Instruction Stream Buffer (ISB) being
full"""
...
class ITLB_Misses:
    ...
    def compute(self, EV):
        try:
            self.val = EV("ICACHE_TAG.STALLS", 3) / CLKS(self, EV, 3) - self.ISB_Full.compute(EV)
            self.thresh = (self.val > 0.05) and self.parent.thresh
        except ZeroDivisionError:
...
class MITE:
    ...
    def compute(self, EV):
        try:
            self.val = (EV("IDQ.MITE_UOPS:c8:i1:eq1", 3) * max(0 , Pipeline_Width - Fetch_MITE(self, EV, 3)) / Pipeline_Width + EV("IDQ.MITE_UOPS", 3) / (EV("IDQ.DSB_UOPS", 3) + EV("IDQ.MITE_UOPS", 3)) * (EV("IDQ_BUBBLES.STARVATION_CYCLES", 3) - EV("IDQ_BUBBLES.FETCH_LATENCY", 3))) / CLKS(self, EV, 3)
            self.thresh = (self.val > 0.1) and self.parent.thresh
        except ZeroDivisionError:
...
class Decoder0_Alone:
    ...
    def compute(self, EV):
        try:
            self.val = EV("INST_DECODED.DECODERS:c1:eq1", 4) / CLKS(self, EV, 4)
            self.thresh = (self.val > 0.1) and self.parent.thresh
        except ZeroDivisionError:
...
class DSB:
    ...
    def compute(self, EV):
        try:
            self.val = (EV("IDQ.DSB_UOPS:c8:i1:eq1", 3) * max(0 , Pipeline_Width - Fetch_DSB(self, EV, 3)) / Pipeline_Width + EV("IDQ.DSB_UOPS", 3) / (EV("IDQ.DSB_UOPS", 3) + EV("IDQ.MITE_UOPS", 3)) * (EV("IDQ_BUBBLES.STARVATION_CYCLES", 3) - EV("IDQ_BUBBLES.FETCH_LATENCY", 3))) / CLKS(self, EV, 3)
...
class MS:
    ...
    def compute(self, EV):
        try:
            self.val = EV("IDQ.MS_CYCLES_ANY", 3) * (Pipeline_Width - EV("IDQ.MS_UOPS", 3) / EV("IDQ.MS_CYCLES_ANY", 3)) / SLOTS(self, EV, 3)
            self.thresh = (self.val > 0.05) and self.parent.thresh
        except ZeroDivisionError:
...
class Bad_Speculation:
    ...
    def compute(self, EV):
        try:
            self.val = (EV("PERF_METRICS.BAD_SPECULATION", 1) / EV("TOPDOWN.SLOTS", 1)) / PERF_METRICS_SUM(self, EV, 1) if topdown_use_fixed else max(1 -(self.Frontend_Bound.compute(EV) + self.Backend_Bound.compute(EV) + self.Retiring.compute(EV)) , 0 )
            self.thresh = (self.val > 0.15)
        except ZeroDivisionError:
...
class Cond_NT_Mispredicts:
    ...
    def compute(self, EV):
        try:
            self.val = (EV("BR_MISP_RETIRED.COND_NTAKEN", 3) / EV("BR_MISP_RETIRED.ALL_BRANCHES", 3)) * self.Branch_Mispredicts.compute(EV)
            self.thresh = (self.val > 0.05) and self.parent.thresh
        except ZeroDivisionError:
...
class Cond_TK_Bwd_Mispredicts:
    name = "Cond_TK_Bwd_Mispredicts"
    domain = "Clocks_Retired"
    area = "BAD"
    level = 3
    htoff = False
    ...
    def compute(self, EV):
        try:
            self.val = (EV("BR_MISP_RETIRED.COND_TAKEN_BWD", 3) / EV("BR_MISP_RETIRED.ALL_BRANCHES", 3)) * self.Branch_Mispredicts.compute(EV)
            self.thresh = (self.val > 0.05) and self.parent.thresh
        except ZeroDivisionError:
            handle_error(self, "Cond_TK_Bwd_Mispredicts zero division")
        return self.val
    desc = """
This metric represents fraction of cycles the CPU was
stalled due to misprediction by backward-taken conditional
branches."""


class Cond_TK_Fwd_Mispredicts:
    name = "Cond_TK_Fwd_Mispredicts"
    domain = "Clocks_Retired"
    area = "BAD"
    level = 3
    htoff = False
    ...
    def compute(self, EV):
        try:
            self.val = (EV("BR_MISP_RETIRED.COND_TAKEN_FWD", 3) / EV("BR_MISP_RETIRED.ALL_BRANCHES", 3)) * self.Branch_Mispredicts.compute(EV)
            self.thresh = (self.val > 0.05) and self.parent.thresh
        except ZeroDivisionError:
            handle_error(self, "Cond_TK_Fwd_Mispredicts zero division")
        return self.val
    desc = """
This metric represents fraction of cycles the CPU was
stalled due to misprediction by forward-taken conditional
branches."""
...
class Ind_Call_Mispredicts:
    ...
    def compute(self, EV):
        try:
            self.val = (EV("BR_MISP_RETIRED.INDIRECT_CALL", 3) / EV("BR_MISP_RETIRED.ALL_BRANCHES", 3)) * self.Branch_Mispredicts.compute(EV)
            self.thresh = (self.val > 0.05) and self.parent.thresh
        except ZeroDivisionError:
...
class Ind_Jump_Mispredicts:
    ...
    def compute(self, EV):
        try:
            self.val = (EV("BR_MISP_RETIRED.INDIRECT_JMP", 3) / EV("BR_MISP_RETIRED.ALL_BRANCHES", 3)) * self.Branch_Mispredicts.compute(EV)
            self.thresh = (self.val > 0.05) and self.parent.thresh
        except ZeroDivisionError:
...
class Ret_Mispredicts:
    ...
    def compute(self, EV):
        try:
            self.val = (EV("BR_MISP_RETIRED.RET", 3) / EV("BR_MISP_RETIRED.ALL_BRANCHES", 3)) * self.Branch_Mispredicts.compute(EV)
            self.thresh = (self.val > 0.05) and self.parent.thresh
        except ZeroDivisionError:
...
class L1_Bound:
    ...
    def compute(self, EV):
        try:
            self.val = EV("MEMORY_STALLS.L1", 3) / CLKS(self, EV, 3)
            self.thresh = (self.val > 0.1) and self.parent.thresh
        except ZeroDivisionError:
...
class Store_Early_Blk:
    name = "Store_Early_Blk"
    domain = "Clocks_Estimated"
    area = "BE/Mem"
    level = 4
    htoff = False
    ...
    def compute(self, EV):
        try:
            self.val = EV("LD_BLOCKS.STORE_EARLY:c1", 4) / CLKS(self, EV, 4)
            self.thresh = (self.val > 0.2)
        except ZeroDivisionError:
            handle_error(self, "Store_Early_Blk zero division")
        return self.val
    desc = """
This metric roughly estimates clocks wasted due to loads
blocked due to unknown store address (did not do memory
disambiguation) or due to unknown store data. GPZv4"""


class L1_Latency_Capacity:
    name = "L1_Latency_Capacity"
    domain = "Clocks_Retired"
    area = "BE/Mem"
    level = 4
    htoff = False
    ...
    def compute(self, EV):
        try:
            self.val = EV("MEM_LOAD_RETIRED.L1_HIT_L1", 4) * min(EV("MEM_LOAD_RETIRED.L1_HIT_L1", 999) , 9) / CLKS(self, EV, 4)
            self.thresh = (self.val > 0.1) and self.parent.thresh
        except ZeroDivisionError:
            handle_error(self, "L1_Latency_Capacity zero division")
        return self.val
    desc = """
This metric estimates fraction of cycles with demand load
accesses that hit Level 1 after missing Level 0 within the
L1D cache."""
...
class FB_Full:
    ...
    def compute(self, EV):
        try:
            self.val = EV("L1D_MISS.FB_FULL", 4) / CLKS(self, EV, 4)
            self.thresh = (self.val > 0.3)
        except ZeroDivisionError:
...
class L2_Bound:
    ...
    def compute(self, EV):
        try:
            self.val = (EV("MEM_LOAD_RETIRED.L2_HIT", 3) / (EV("MEM_LOAD_RETIRED.L2_HIT", 3) + EV("MEM_LOAD_MISC_RETIRED.UC", 3) * UC_Scale_Factor)) * EV("MEMORY_STALLS.L2", 3) / CLKS(self, EV, 3)
            self.thresh = (self.val > 0.05) and self.parent.thresh
        except ZeroDivisionError:
...
class L3_Bound:
    ...
    def compute(self, EV):
        try:
            self.val = EV("MEMORY_STALLS.L3", 3) / CLKS(self, EV, 3)
            self.thresh = (self.val > 0.05) and self.parent.thresh
        except ZeroDivisionError:
...
class Contested_Accesses:
    ...
    def compute(self, EV):
        try:
            self.val = (EV("MEM_LOAD_L3_HIT_RETIRED.XSNP_MISS", 4) * min(EV("MEM_LOAD_L3_HIT_RETIRED.XSNP_MISS", 999) , Mem_XSNP_Hit_Cost(self, EV, 4) - Mem_L2_Hit_Cost(self, EV, 4)) + EV("MEM_LOAD_L3_HIT_RETIRED.XSNP_HITM", 4) * min(EV("MEM_LOAD_L3_HIT_RETIRED.XSNP_HITM", 999) , Mem_XSNP_HitM_Cost(self, EV, 4) - Mem_L2_Hit_Cost(self, EV, 4))) * FB_Factor(self, EV, 4) / CLKS(self, EV, 4)
            self.thresh = (self.val > 0.05) and self.parent.thresh
        except ZeroDivisionError:
...
class Data_Sharing:
    ...
    def compute(self, EV):
        try:
            self.val = (EV("MEM_LOAD_L3_HIT_RETIRED.XSNP_NO_FWD", 4) * min(EV("MEM_LOAD_L3_HIT_RETIRED.XSNP_NO_FWD", 999) , Mem_XSNP_Hit_Cost(self, EV, 4) - Mem_L2_Hit_Cost(self, EV, 4)) + EV("MEM_LOAD_L3_HIT_RETIRED.XSNP_FWD", 4) * min(EV("MEM_LOAD_L3_HIT_RETIRED.XSNP_FWD", 999) , Mem_XSNP_HitM_Cost(self, EV, 4) - Mem_L2_Hit_Cost(self, EV, 4))) * FB_Factor(self, EV, 4) / CLKS(self, EV, 4)
            self.thresh = (self.val > 0.05) and self.parent.thresh
        except ZeroDivisionError:
    ...
    desc = """
This metric estimates fraction of cycles while the memory
subsystem was handling synchronizations due to L3 data-
sharing accesses. Data shared by multiple Logical Processors
(even just read shared) may cause increased access latency
due to cache coherency. Excessive data sharing can
drastically harm multithreaded performance."""
...
class SQ_Full:
    ...
    def compute(self, EV):
        try:
            self.val = (EV("XQ.FULL", 4) + EV("L1D_MISS.L2_STALLS", 4)) / CLKS(self, EV, 4)
...
class L3_Miss_Bound:
    ...
    errcount = 0
    sibling = None
    metricgroup = frozenset(['MemoryBound', 'TmaL3mem', 'Offcore'])
    maxval = 1
    def compute(self, EV):
        try:
            ...
            handle_error(self, "L3_Miss_Bound zero division")
        return self.val
    desc = """
...
class UC_Bound:
    ...
    def compute(self, EV):
        try:
            self.val = (EV("MEM_LOAD_MISC_RETIRED.UC", 4) * UC_Scale_Factor /(EV("MEM_LOAD_RETIRED.L2_HIT", 4) + EV("MEM_LOAD_MISC_RETIRED.UC", 4) * UC_Scale_Factor)) * EV("MEMORY_STALLS.L2", 4) / CLKS(self, EV, 4)
            self.thresh = (self.val > 0.2) and self.parent.thresh
        except ZeroDivisionError:
            handle_error(self, "UC_Bound zero division")
        return self.val
    desc = """
This metric roughly estimates fraction of cycles where the
core's performance was likely hurt due to un-chacheable
memory"""
...
class Store_STLB_Miss:
    ...
    def compute(self, EV):
        try:
            self.val = EV("DTLB_STORE_MISSES.WALK_ACTIVE", 5) / CLKS(self, EV, 5)
            self.thresh = (self.val > 0.05) and self.parent.thresh
        except ZeroDivisionError:
...
class Serializing_Operation:
    ...
    def compute(self, EV):
        try:
            self.val = (EV("BE_STALLS.SCOREBOARD", 3) + EV("CPU_CLK_UNHALTED.C02", 3)) / CLKS(self, EV, 3)
            self.thresh = (self.val > 0.1) and self.parent.thresh
        except ZeroDivisionError:
...
class ALU_Op_Utilization:
    ...
    def compute(self, EV):
        try:
            self.val = EV("UOPS_DISPATCHED.ALU", 5) / (6 * CLKS(self, EV, 5))
...
class Load_Op_Utilization:
    ...
    def compute(self, EV):
        try:
            self.val = EV("UOPS_DISPATCHED.LOAD", 5) / (3 * CLKS(self, EV, 5))
...
class Store_Op_Utilization:
    ...
    level = 5
    htoff = False
    sample = ['UOPS_DISPATCHED.STD', 'UOPS_DISPATCHED.STA']
    errcount = 0
    sibling = None
    metricgroup = frozenset([])
    maxval = None
    def compute(self, EV):
        try:
            self.val = (EV("UOPS_DISPATCHED.STD", 5) + EV("UOPS_DISPATCHED.STA", 5)) / (7 * CLKS(self, EV, 5))
            self.thresh = (self.val > 0.6)
        except ZeroDivisionError:
...
class FP_Vector_128b:
    ...
    def compute(self, EV):
        try:
            self.val = (EV("FP_ARITH_OPS_RETIRED.128B_PACKED_DOUBLE", 5) + EV("FP_ARITH_OPS_RETIRED.128B_PACKED_SINGLE", 5)) / Retired_Slots(self, EV, 5)
            self.thresh = (self.val > 0.1) and self.parent.thresh
        except ZeroDivisionError:
...
class FP_Vector_256b:
    ...
    def compute(self, EV):
        try:
            self.val = EV("FP_ARITH_OPS_RETIRED.VECTOR_256B", 5) / Retired_Slots(self, EV, 5)
            self.thresh = (self.val > 0.1) and self.parent.thresh
        except ZeroDivisionError:
...
class Int_Vector_128b:
    ...
    def compute(self, EV):
        try:
            self.val = EV("INT_VEC_RETIRED.128BIT", 4) / Retired_Slots(self, EV, 4)
            self.thresh = (self.val > 0.1) and self.parent.thresh
        except ZeroDivisionError:
...
class Int_Vector_256b:
    ...
    def compute(self, EV):
        try:
            self.val = EV("INT_VEC_RETIRED.256BIT", 4) / Retired_Slots(self, EV, 4)
            self.thresh = (self.val > 0.1) and self.parent.thresh
        except ZeroDivisionError:
...
class Non_Fused_Branches:
    ...
    def compute(self, EV):
        try:
            self.val = self.Light_Operations.compute(EV) * (EV("BR_INST_RETIRED.ALL_BRANCHES", 3) - EV("INST_RETIRED.BR_FUSED", 3)) / Retired_Slots(self, EV, 3)
            self.thresh = (self.val > 0.1) and self.parent.thresh
        except ZeroDivisionError:
...
class Metric_Data_Cache_Memory_Bandwidth:
    name = "Data_Cache_Memory_Bandwidth"
    domain = "Scaled_Slots"
    maxval = 0
    errcount = 0
    area = "Bottleneck"
    ...
    def compute(self, EV):
        try:
            self.val = Data_Cache_Memory_Bandwidth(self, EV, 0)
            self.thresh = (self.val > 20)
        except ZeroDivisionError:
            handle_error_metric(self, "Data_Cache_Memory_Bandwidth zero division")
    desc = """
Total pipeline cost of external Memory- or Cache-Bandwidth
related bottlenecks"""


class Metric_Data_Cache_Memory_Latency:
    name = "Data_Cache_Memory_Latency"
    domain = "Scaled_Slots"
    maxval = 0
    errcount = 0
    area = "Bottleneck"
    ...
    def compute(self, EV):
        try:
            self.val = Data_Cache_Memory_Latency(self, EV, 0)
            self.thresh = (self.val > 20)
        except ZeroDivisionError:
            handle_error_metric(self, "Data_Cache_Memory_Latency zero division")
    desc = """
...
class Metric_VectorMixPKI:
    ...
    def compute(self, EV):
        try:
            self.val = VectorMixPKI(self, EV, 0)
            self.thresh = (self.val > 0.05)
        except ZeroDivisionError:
            handle_error_metric(self, "VectorMixPKI zero division")
    desc = """
AVX preserve/restore assists per kilo instruction"""
...
class Metric_Retire_Stall:
    ...
    def compute(self, EV):
        try:
            self.val = Retire_Stall(self, EV, 0)
            self.thresh = True
        except ZeroDivisionError:
            handle_error_metric(self, "Retire_Stall zero division")
    desc = """
Total percent of cycles without actually retired uops"""
...
class Metric_Fetch_MS:
    ...
    def compute(self, EV):
        try:
            self.val = Fetch_MS(self, EV, 0)
            self.thresh = True
        except ZeroDivisionError:
            handle_error_metric(self, "Fetch_MS zero division")
    desc = """
Average number of uops fetched from MS per cycle"""
...
class Metric_IpMisp_Cond_Taken_Bwd:
    name = "IpMisp_Cond_Taken_Bwd"
    domain = ""
    maxval = 0
    errcount = 0
    area = "Info.Bad_Spec"
    ...
    def compute(self, EV):
        try:
            self.val = IpMisp_Cond_Taken_Bwd(self, EV, 0)
            self.thresh = True
        except ZeroDivisionError:
            handle_error_metric(self, "IpMisp_Cond_Taken_Bwd zero division")
    desc = """
Instructions per retired Mispredicts for conditional
backward-taken branches (lower number means higher
occurrence rate)."""


class Metric_IpMisp_Cond_Taken_Fwd:
    name = "IpMisp_Cond_Taken_Fwd"
    domain = ""
    maxval = 0
    errcount = 0
    area = "Info.Bad_Spec"
    ...
    def compute(self, EV):
        try:
            self.val = IpMisp_Cond_Taken_Fwd(self, EV, 0)
            self.thresh = True
        except ZeroDivisionError:
            handle_error_metric(self, "IpMisp_Cond_Taken_Fwd zero division")
    desc = """
Instructions per retired Mispredicts for conditional
forward-taken branches (lower number means higher occurrence
rate)."""
...
class Metric_Cond_TK_Bwd:
    ...
    def compute(self, EV):
        try:
            self.val = Cond_TK_Bwd(self, EV, 0)
            self.thresh = (self.val > 0.3)
        except ZeroDivisionError:
            handle_error_metric(self, "Cond_TK_Bwd zero division")
    desc = """
Fraction of branches that are forward taken conditionals"""


class Metric_Cond_TK_Fwd:
    name = "Cond_TK_Fwd"
    domain = "Fraction"
    maxval = 1
    errcount = 0
    ...
    def compute(self, EV):
        try:
            self.val = Cond_TK_Fwd(self, EV, 0)
            self.thresh = (self.val > 0.2)
        except ZeroDivisionError:
            handle_error_metric(self, "Cond_TK_Fwd zero division")
    desc = """
Fraction of branches that are forward taken conditionals"""
...
class Metric_L1DL0_MPKI:
    ...
    def compute(self, EV):
        try:
            self.val = L1DL0_MPKI(self, EV, 0)
            self.thresh = True
        except ZeroDivisionError:
            handle_error_metric(self, "L1DL0_MPKI zero division")
    desc = """
L0 cache true misses per kilo instruction for retired demand
loads"""
...
class Metric_L1DL0_Cache_Fill_BW:
    name = "L1DL0_Cache_Fill_BW"
    domain = "Metric"
    maxval = 0
    errcount = 0
    area = "Info.Memory"
    ...
    def compute(self, EV):
        try:
            self.val = L1DL0_Cache_Fill_BW(self, EV, 0)
            self.thresh = True
        except ZeroDivisionError:
            handle_error_metric(self, "L1DL0_Cache_Fill_BW zero division")
    desc = """
Average per-thread data fill bandwidth to the Level 0 within
L1D cache [GB / sec]"""
...
class Setup:
    def __init__(self, r):
        ...
        n = ISB_Full() ; r.run(n) ; o["ISB_Full"] = n
        ...
        n = Cond_TK_Bwd_Mispredicts() ; r.run(n) ; o["Cond_TK_Bwd_Mispredicts"] = n
        n = Cond_TK_Fwd_Mispredicts() ; r.run(n) ; o["Cond_TK_Fwd_Mispredicts"] = n
        ...
        n = Store_Early_Blk() ; r.run(n) ; o["Store_Early_Blk"] = n
        n = L1_Latency_Capacity() ; r.run(n) ; o["L1_Latency_Capacity"] = n
        ...
        o["ISB_Full"].parent = o["ICache_Misses"]
        ...
        o["Cond_TK_Bwd_Mispredicts"].parent = o["Branch_Mispredicts"]
        o["Cond_TK_Fwd_Mispredicts"].parent = o["Branch_Mispredicts"]
        ...
        o["Store_Early_Blk"].parent = o["L1_Bound"]
        o["L1_Latency_Capacity"].parent = o["L1_Bound"]
        ...
        o["L3_Miss_Bound"].parent = o["Memory_Bound"]
        o["MEM_Bandwidth"].parent = o["L3_Miss_Bound"]
        o["MEM_Latency"].parent = o["L3_Miss_Bound"]
        o["UC_Bound"].parent = o["L3_Miss_Bound"]
        ...
        n = Metric_Data_Cache_Memory_Bandwidth() ; r.metric(n) ; o["Data_Cache_Memory_Bandwidth"] = n
        n = Metric_Data_Cache_Memory_Latency() ; r.metric(n) ; o["Data_Cache_Memory_Latency"] = n
        ...
        n = Metric_VectorMixPKI() ; r.metric(n) ; o["VectorMixPKI"] = n
        ...
        n = Metric_Retire_Stall() ; r.metric(n) ; o["Retire_Stall"] = n
        ...
        n = Metric_Fetch_MS() ; r.metric(n) ; o["Fetch_MS"] = n
        ...
        n = Metric_IpMisp_Cond_Taken_Bwd() ; r.metric(n) ; o["IpMisp_Cond_Taken_Bwd"] = n
        n = Metric_IpMisp_Cond_Taken_Fwd() ; r.metric(n) ; o["IpMisp_Cond_Taken_Fwd"] = n
        ...
        n = Metric_Cond_TK_Bwd() ; r.metric(n) ; o["Cond_TK_Bwd"] = n
        n = Metric_Cond_TK_Fwd() ; r.metric(n) ; o["Cond_TK_Fwd"] = n
        ...
        n = Metric_L1DL0_MPKI() ; r.metric(n) ; o["L1DL0_MPKI"] = n
        ...
        n = Metric_L1DL0_Cache_Fill_BW() ; r.metric(n) ; o["L1DL0_Cache_Fill_BW"] = n
        ...
        o["ICache_Misses"].ISB_Full = o["ISB_Full"]
        o["Code_L2_Hit"].Code_L2_Miss = o["Code_L2_Miss"]
        o["ITLB_Misses"].ISB_Full = o["ISB_Full"]
        ...
        o["Cond_NT_Mispredicts"].Branch_Mispredicts = o["Branch_Mispredicts"]
        o["Cond_TK_Bwd_Mispredicts"].Branch_Mispredicts = o["Branch_Mispredicts"]
        o["Cond_TK_Fwd_Mispredicts"].Branch_Mispredicts = o["Branch_Mispredicts"]
        o["Ind_Call_Mispredicts"].Branch_Mispredicts = o["Branch_Mispredicts"]
        o["Ind_Jump_Mispredicts"].Branch_Mispredicts = o["Branch_Mispredicts"]
        o["Ret_Mispredicts"].Branch_Mispredicts = o["Branch_Mispredicts"]
        ...
        o["Mispredictions"].ISB_Full = o["ISB_Full"]
        ...
        o["Big_Code"].ISB_Full = o["ISB_Full"]
        ...
        o["Instruction_Fetch_BW"].ISB_Full = o["ISB_Full"]
        ...
        o["Data_Cache_Memory_Bandwidth"].L1_Bound = o["L1_Bound"]
        o["Data_Cache_Memory_Bandwidth"].L3_Bound = o["L3_Bound"]
        o["Data_Cache_Memory_Bandwidth"].Store_Bound = o["Store_Bound"]
        o["Data_Cache_Memory_Bandwidth"].Memory_Bound = o["Memory_Bound"]
        o["Data_Cache_Memory_Bandwidth"].MEM_Latency = o["MEM_Latency"]
        o["Data_Cache_Memory_Bandwidth"].Data_Sharing = o["Data_Sharing"]
        o["Data_Cache_Memory_Bandwidth"].L3_Miss_Bound = o["L3_Miss_Bound"]
        o["Data_Cache_Memory_Bandwidth"].Store_Early_Blk = o["Store_Early_Blk"]
        o["Data_Cache_Memory_Bandwidth"].UC_Bound = o["UC_Bound"]
        o["Data_Cache_Memory_Bandwidth"].Contested_Accesses = o["Contested_Accesses"]
        o["Data_Cache_Memory_Bandwidth"].Load_STLB_Miss = o["Load_STLB_Miss"]
        o["Data_Cache_Memory_Bandwidth"].L3_Hit_Latency = o["L3_Hit_Latency"]
        o["Data_Cache_Memory_Bandwidth"].FB_Full = o["FB_Full"]
        o["Data_Cache_Memory_Bandwidth"].SQ_Full = o["SQ_Full"]
        o["Data_Cache_Memory_Bandwidth"].DTLB_Load = o["DTLB_Load"]
        o["Data_Cache_Memory_Bandwidth"].L2_Bound = o["L2_Bound"]
        o["Data_Cache_Memory_Bandwidth"].Split_Loads = o["Split_Loads"]
        o["Data_Cache_Memory_Bandwidth"].MEM_Bandwidth = o["MEM_Bandwidth"]
        o["Data_Cache_Memory_Bandwidth"].Store_Fwd_Blk = o["Store_Fwd_Blk"]
        o["Data_Cache_Memory_Bandwidth"].Lock_Latency = o["Lock_Latency"]
        o["Data_Cache_Memory_Bandwidth"].L1_Latency_Capacity = o["L1_Latency_Capacity"]
        o["Data_Cache_Memory_Latency"].FB_Full = o["FB_Full"]
        o["Data_Cache_Memory_Latency"].L1_Bound = o["L1_Bound"]
        o["Data_Cache_Memory_Latency"].L3_Bound = o["L3_Bound"]
        o["Data_Cache_Memory_Latency"].Load_STLB_Miss = o["Load_STLB_Miss"]
        o["Data_Cache_Memory_Latency"].Store_Bound = o["Store_Bound"]
        o["Data_Cache_Memory_Latency"].Memory_Bound = o["Memory_Bound"]
        o["Data_Cache_Memory_Latency"].SQ_Full = o["SQ_Full"]
        o["Data_Cache_Memory_Latency"].MEM_Latency = o["MEM_Latency"]
        o["Data_Cache_Memory_Latency"].DTLB_Load = o["DTLB_Load"]
        o["Data_Cache_Memory_Latency"].Data_Sharing = o["Data_Sharing"]
        o["Data_Cache_Memory_Latency"].L3_Miss_Bound = o["L3_Miss_Bound"]
        o["Data_Cache_Memory_Latency"].L2_Bound = o["L2_Bound"]
        o["Data_Cache_Memory_Latency"].Split_Loads = o["Split_Loads"]
        o["Data_Cache_Memory_Latency"].Store_Early_Blk = o["Store_Early_Blk"]
        o["Data_Cache_Memory_Latency"].MEM_Bandwidth = o["MEM_Bandwidth"]
        o["Data_Cache_Memory_Latency"].UC_Bound = o["UC_Bound"]
        o["Data_Cache_Memory_Latency"].Contested_Accesses = o["Contested_Accesses"]
        o["Data_Cache_Memory_Latency"].Store_Fwd_Blk = o["Store_Fwd_Blk"]
        o["Data_Cache_Memory_Latency"].Lock_Latency = o["Lock_Latency"]
        o["Data_Cache_Memory_Latency"].L3_Hit_Latency = o["L3_Hit_Latency"]
        o["Data_Cache_Memory_Latency"].L1_Latency_Capacity = o["L1_Latency_Capacity"]
        o["Memory_Data_TLBs"].L1_Bound = o["L1_Bound"]
        o["Memory_Data_TLBs"].L3_Bound = o["L3_Bound"]
        o["Memory_Data_TLBs"].L1_Latency_Capacity = o["L1_Latency_Capacity"]
        o["Memory_Data_TLBs"].Store_Bound = o["Store_Bound"]
        o["Memory_Data_TLBs"].Memory_Bound = o["Memory_Bound"]
        o["Memory_Data_TLBs"].L3_Miss_Bound = o["L3_Miss_Bound"]
        o["Memory_Data_TLBs"].Store_Early_Blk = o["Store_Early_Blk"]
        ...
        o["Memory_Synchronization"].L3_Miss_Bound = o["L3_Miss_Bound"]
        ...
        o["Irregular_Overhead"].ISB_Full = o["ISB_Full"]
        ...
        o["Other_Bottlenecks"].L1_Latency_Capacity = o["L1_Latency_Capacity"]
        ...
        o["Other_Bottlenecks"].L3_Miss_Bound = o["L3_Miss_Bound"]
        ...
        o["Other_Bottlenecks"].ISB_Full = o["ISB_Full"]
        ...
        o["Other_Bottlenecks"].Store_Early_Blk = o["Store_Early_Blk"]
        ...
        o["Other_Bottlenecks"].UC_Bound = o["UC_Bound"]
        ...
        o["DSB_Misses"].ISB_Full = o["ISB_Full"]
        ...
        o["IC_Misses"].ISB_Full = o["ISB_Full"]
        ...
        o["Branch_Misprediction_Cost"].ISB_Full = o["ISB_Full"]
        ...
        o["FB_Full"].sibling = (o["SQ_Full"], o["MEM_Bandwidth"], o["UC_Bound"], o["Store_Latency"], o["Streaming_Stores"],)
        ...
        o["SQ_Full"].sibling = (o["FB_Full"], o["MEM_Bandwidth"], o["UC_Bound"],)
        o["MEM_Bandwidth"].sibling = (o["FB_Full"], o["SQ_Full"], o["UC_Bound"],)
        o["MEM_Latency"].sibling = (o["L3_Hit_Latency"],)
        o["UC_Bound"].sibling = (o["FB_Full"], o["SQ_Full"], o["MEM_Bandwidth"],)
        ...
        o["Ports_Utilized_2"].sibling = (o["FP_Scalar"], o["FP_Vector"], o["FP_Vector_128b"], o["FP_Vector_256b"], o["Int_Vector_128b"], o["Int_Vector_256b"],)
        o["FP_Scalar"].sibling = (o["Ports_Utilized_2"], o["FP_Vector"], o["FP_Vector_128b"], o["FP_Vector_256b"], o["Int_Vector_128b"], o["Int_Vector_256b"],)
        o["FP_Vector"].sibling = (o["Ports_Utilized_2"], o["FP_Scalar"], o["FP_Vector_128b"], o["FP_Vector_256b"], o["Int_Vector_128b"], o["Int_Vector_256b"],)
        o["FP_Vector_128b"].sibling = (o["Ports_Utilized_2"], o["FP_Scalar"], o["FP_Vector"], o["FP_Vector_256b"], o["Int_Vector_128b"], o["Int_Vector_256b"],)
        o["FP_Vector_256b"].sibling = (o["Ports_Utilized_2"], o["FP_Scalar"], o["FP_Vector"], o["FP_Vector_128b"], o["Int_Vector_128b"], o["Int_Vector_256b"],)
        o["Int_Vector_128b"].sibling = (o["Ports_Utilized_2"], o["FP_Scalar"], o["FP_Vector"], o["FP_Vector_128b"], o["FP_Vector_256b"], o["Int_Vector_256b"],)
        o["Int_Vector_256b"].sibling = (o["Ports_Utilized_2"], o["FP_Scalar"], o["FP_Vector"], o["FP_Vector_128b"], o["FP_Vector_256b"], o["Int_Vector_128b"],)
        ...
        o["Data_Cache_Memory_Bandwidth"].sibling = (o["FB_Full"], o["SQ_Full"], o["MEM_Bandwidth"], o["UC_Bound"],)
        o["Data_Cache_Memory_Latency"].sibling = (o["L3_Hit_Latency"], o["MEM_Latency"],)
        ...
        o["VectorMixPKI"].sibling = (o["MS_Switches"], o["Mixing_Vectors"],)
        ...
        o["DRAM_BW_Use"].sibling = (o["FB_Full"], o["SQ_Full"], o["MEM_Bandwidth"], o["UC_Bound"],)
# missing nodes: Power
