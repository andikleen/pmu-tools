...
_re_bracket_event = re.compile(r'\s+\[.*\]')
_re_cpu_event = re.compile(r'(cpu_core|cpu_atom|cpu|tool)/(cycles|duration_time)/')
_re_numeric_count = re.compile(r"\s*[0-9.]+")
_re_angle_count = re.compile(r"\s*<")
_re_uncore_event = re.compile(r'power|uncore')
_re_coretitle = re.compile(r'(S\d+-)?(D\d+-)?C\d+')
_re_coretitle_match = re.compile(r'(?:S(\d+)-)?(?:D(\d+)-)?C(\d+)')
_re_remove_qual_colon = re.compile(r':(p?)[kuhgHeD]+$')
_re_remove_qual_slash = re.compile(r'/(p?)[kuhgHeD]+$')
_re_remove_percore = re.compile(r',percore=\d+')
...
def remove_qual(ev):
    def get_group(prefix, m):
        ...
        return prefix
    return _re_remove_percore.sub("", _re_remove_qual_colon.sub(lambda m: get_group("", m), _re_remove_qual_slash.sub(lambda m: get_group("/", m), ev)))
...
class ValidEvents(object):
    def update(self):
        ...
        self.compiled = re.compile(self.string, re.I)
...
def is_event(l, n):
    ...
    return valid_events.compiled.match(l[n])

def is_number(n):
    return _re_digits.match(n) is not None
...
def do_execute(rlist, summary, evstr, flat_rmap, out, rest, resoff, revnum):
    ...
    while True:
        ...
        event = _re_bracket_event.sub('', event)
        event = remove_qual(event)
        m = _re_cpu_event.match(event)
        if m:
            event = m.group(2)
        ...
        multiplex = float('nan')
        event = event.rstrip()
        if _re_numeric_count.match(count):
            val = float(count.replace(",", "."))
        elif _re_angle_count.match(count):
        ...
        def uncore_event(event):
            return _re_uncore_event.match(event)
        ...
        elif _re_coretitle.match(title) and (smt_mode or args.no_aggr):
            m = _re_coretitle_match.match(title)
        ...
        if args.raw or args.valcsv:
            dump_raw(out.valcsv,
                     ...
                     val if val or not _re_angle_count.match(count) else count,
                     len(res[title]) - 1,
