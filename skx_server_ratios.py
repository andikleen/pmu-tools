...
#
# auto generated TopDown/TMAM 4.1 description for Intel Xeon Scalable Processors (code named Skylake Server)
...
def Mispredictions(self, EV, level):
    return 100 *(self.Branch_Mispredicts.compute(EV) + self.Fetch_Latency.compute(EV) * self.Mispredicts_Resteers.compute(EV) / (self.LCP.compute(EV) + self.ICache_Misses.compute(EV) + self.DSB_Switches.compute(EV) + self.Branch_Resteers.compute(EV) + self.Clears_Resteers.compute(EV) + self.MS_Switches.compute(EV) + self.ITLB_Misses.compute(EV) + self.Unknown_Branches.compute(EV) + self.Mispredicts_Resteers.compute(EV)))
...
def IO_Write_BW(self, EV, level):
    return (EV("UNC_IIO_DATA_REQ_OF_CPU.MEM_READ.PART0", level) + EV("UNC_IIO_DATA_REQ_OF_CPU.MEM_READ.PART1", level) + EV("UNC_IIO_DATA_REQ_OF_CPU.MEM_READ.PART2", level) + EV("UNC_IIO_DATA_REQ_OF_CPU.MEM_READ.PART3", level)) * 4 / OneBillion / EV("interval-s", 0)

# Average IO (network or disk) Bandwidth Use for Reads [GB / sec]
def IO_Read_BW(self, EV, level):
    return (EV("UNC_IIO_DATA_REQ_OF_CPU.MEM_WRITE.PART0", level) + EV("UNC_IIO_DATA_REQ_OF_CPU.MEM_WRITE.PART1", level) + EV("UNC_IIO_DATA_REQ_OF_CPU.MEM_WRITE.PART2", level) + EV("UNC_IIO_DATA_REQ_OF_CPU.MEM_WRITE.PART3", level)) * 4 / OneBillion / EV("interval-s", 0)
...
under Frontend Bound."""
    level = 1
    htoff = False
    sample = ['FRONTEND_RETIRED.LATENCY_GE_4']
    errcount = 0
    sibling = None
    server = False
...
some period."""
    level = 2
    htoff = False
    sample = ['FRONTEND_RETIRED.LATENCY_GE_16', 'FRONTEND_RETIRED.LATENCY_GE_8']
    errcount = 0
    sibling = None
    server = False
...
stalled due to instruction cache misses."""
    level = 3
    htoff = False
    sample = ['FRONTEND_RETIRED.L2_MISS', 'FRONTEND_RETIRED.L1I_MISS']
    errcount = 0
    sibling = None
    server = False
...
stalled due to Instruction TLB (ITLB) misses."""
    level = 3
    htoff = False
    sample = ['FRONTEND_RETIRED.STLB_MISS', 'FRONTEND_RETIRED.ITLB_MISS']
    errcount = 0
    sibling = None
    server = False
...
considered."""
    level = 3
    htoff = False
    sample = ['FRONTEND_RETIRED.DSB_MISS']
    errcount = 0
    sibling = None
    server = False
...
without having that demand load missing the L1 cache."""
    level = 3
    htoff = False
    sample = ['MEM_LOAD_RETIRED.L1_HIT', 'MEM_LOAD_RETIRED.FB_HIT']
    errcount = 0
    sibling = None
    server = False
...
as well as performing a hardware page walk on an STLB miss."""
    level = 4
    htoff = False
    sample = ['MEM_INST_RETIRED.STLB_MISS_LOADS']
    errcount = 0
    sibling = None
    server = False
    metricgroup = ['TLB']
...
boundary."""
    level = 4
    htoff = False
    sample = ['MEM_INST_RETIRED.SPLIT_LOADS']
    errcount = 0
    sibling = None
    server = False
    metricgroup = []
...
the latency and increase performance."""
    level = 3
    htoff = False
    sample = ['MEM_LOAD_RETIRED.L3_HIT']
    errcount = 0
    sibling = None
    server = False
    metricgroup = ['Cache_Misses', 'Memory_Bound']
...
such as modified locked variables; and false sharing."""
    level = 4
    htoff = False
    sample = ['MEM_LOAD_L3_HIT_RETIRED.XSNP_HITM', 'MEM_LOAD_L3_HIT_RETIRED.XSNP_MISS']
    errcount = 0
    sibling = None
    server = False
    ...
    def compute(self, EV):
        try:
            self.val = (Mem_XSNP_HitM_Cost * LOAD_XSNP_HITM(self, EV, 4) + MEM_XSNP_Hit_Cost * LOAD_XSNP_MISS(self, EV, 4)) * FBHit_Factor(self, EV, 4) / CLKS(self, EV, 4)
            self.thresh = (self.val > 0.2) & self.parent.thresh
        except ZeroDivisionError:
...
multithreaded performance."""
    level = 4
    htoff = False
    sample = ['MEM_LOAD_L3_HIT_RETIRED.XSNP_HIT']
    errcount = 0
    sibling = None
    server = False
    metricgroup = ['Offcore']
    def compute(self, EV):
        try:
            self.val = MEM_XSNP_Hit_Cost * LOAD_XSNP_HIT(self, EV, 4) * FBHit_Factor(self, EV, 4) / CLKS(self, EV, 4)
            self.thresh = (self.val > 0.2) & self.parent.thresh
        except ZeroDivisionError:
...
its siblings."""
    ...
    def compute(self, EV):
        try:
            self.val = MEM_XSNP_None_Cost * LOAD_L3_HIT(self, EV, 4) * FBHit_Factor(self, EV, 4) / CLKS(self, EV, 4)
            self.thresh = (self.val > 0.1) & self.parent.thresh
        except ZeroDivisionError:
...
improve the latency and increase performance."""
    ...
    def compute(self, EV):
        try:
            self.val = Mem_Local_DRAM_Cost * EV("MEM_LOAD_L3_MISS_RETIRED.LOCAL_DRAM", 5) * FBHit_Factor(self, EV, 5) / CLKS(self, EV, 5)
            self.thresh = (self.val > 0.1) & self.parent.thresh
        except ZeroDivisionError:
...
caused often due to non-optimal NUMA allocations."""
    level = 5
    htoff = False
    sample = ['MEM_LOAD_L3_MISS_RETIRED.REMOTE_DRAM']
    errcount = 0
    sibling = None
    server = False
    metricgroup = ['Server']
    def compute(self, EV):
        try:
            self.val = Mem_Remote_DRAM_Cost * EV("MEM_LOAD_L3_MISS_RETIRED.REMOTE_DRAM", 5) * FBHit_Factor(self, EV, 5) / CLKS(self, EV, 5)
            self.thresh = (self.val > 0.1) & self.parent.thresh
        except ZeroDivisionError:
...
often due to non-optimal NUMA allocations."""
    level = 5
    htoff = False
    sample = ['MEM_LOAD_L3_MISS_RETIRED.REMOTE_HITM', 'MEM_LOAD_L3_MISS_RETIRED.REMOTE_FWD']
    errcount = 0
    sibling = None
    server = False
    ...
    def compute(self, EV):
        try:
            self.val = (Mem_Remote_HitM_Cost * EV("MEM_LOAD_L3_MISS_RETIRED.REMOTE_HITM", 5) + Mem_Remote_Fwd_Cost * EV("MEM_LOAD_L3_MISS_RETIRED.REMOTE_FWD", 5)) * FBHit_Factor(self, EV, 5) / CLKS(self, EV, 5)
            self.thresh = (self.val > 0.1) & self.parent.thresh
        except ZeroDivisionError:
...
flagged should any of these cases be a bottleneck."""
    level = 3
    htoff = False
    sample = ['MEM_INST_RETIRED.ALL_STORES']
    errcount = 0
    sibling = None
    server = False
    metricgroup = ['Memory_Bound']
...
cache line."""
    level = 4
    htoff = False
    sample = ['MEM_LOAD_L3_HIT_RETIRED.XSNP_HITM', 'OFFCORE_RESPONSE.DEMAND_RFO.L3_HIT.HITM_OTHER_CORE']
    errcount = 0
    sibling = None
    server = False
    metricgroup = ['Data_Sharing', 'Offcore']
...
granularity."""
    level = 4
    htoff = False
    sample = ['MEM_INST_RETIRED.SPLIT_STORES']
    errcount = 0
    sibling = None
    server = False
    metricgroup = []
...
used data."""
    level = 4
    htoff = False
    sample = ['MEM_INST_RETIRED.STLB_MISS_STORES']
    errcount = 0
    sibling = None
    server = False
    metricgroup = ['TLB']
...
class Setup:
    def __init__(self, r):
        ...
        o["Mispredictions"].Clears_Resteers = o["Clears_Resteers"]
