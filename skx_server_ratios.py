...
#
# auto generated TopDown/TMAM 3.2 description for Intel Xeon Scalable Processors (code named Skylake Server)
...
def Avg_RS_Empty_Period_Clears(self, EV, level):
    return (EV("RS_EVENTS.EMPTY_CYCLES", level) - EV("ICACHE_16B.IFDATA_STALL", level) - EV("ICACHE_64B.IFTAG_STALL", level)) / EV("RS_EVENTS.EMPTY_END", level)
...
def HighIPC(self, EV, level):
    return (IPC(self, EV, level) / Pipeline_Width) >= 0
...
def IPArith(self, EV, level):
    return IPArith_Scalar_SP(self, EV, level) + IPArith_Scalar_DP(self, EV, level) + IPArith_AVX128(self, EV, level) + IPArith_AVX256(self, EV, level) + IPArith_AVX512(self, EV, level)
...
def BAClear_Cost(self, EV, level):
    return 2 * Avg_RS_Empty_Period_Clears(self, EV, level)
...
def ITLB_Misses_Legacy(self, EV, level):
...
the Backend (less than four)."""
    ...
    def compute(self, EV):
        try:
            ...
            self.thresh = (self.val > 0.1) & self.parent.thresh & HighIPC(self, EV, 2)
        except ZeroDivisionError:
...
lines."""
    level = 4
    htoff = False
    sample = ['MEM_LOAD_L3_HIT_RETIRED.XSNP_HITM:pp', 'OFFCORE_RESPONSE:request=DEMAND_RFO:response=L3_HIT.HITM_OTHER_CORE']
    errcount = 0
    sibling = None
    server = False
    metricgroup = ['Data_Sharing']
    def compute(self, EV):
        try:
            self.val = (Mem_Remote_HitM_Cost * EV("OFFCORE_RESPONSE:request=DEMAND_RFO:response=L3_MISS.REMOTE_HITM", 4) + Mem_XSNP_HitM_Cost * EV("OFFCORE_RESPONSE:request=DEMAND_RFO:response=L3_HIT.HITM_OTHER_CORE", 4)) / CLKS(self, EV, 4 )
            self.thresh = (self.val > 0.1) & self.parent.thresh
        except ZeroDivisionError:
...
class Metric_ITLB_Misses_Legacy:
    name = "ITLB_Misses_Legacy"
    desc = """
...
misses using the legacy bottom-up calculation."""
    ...
    def compute(self, EV):
        try:
            self.val = ITLB_Misses_Legacy(self, EV, 0)
        except ZeroDivisionError:
            print_error("ITLB_Misses_Legacy zero division")
            self.errcount += 1
            self.val = 0
...
codes."""
    ...
    def compute(self, EV):
        try:
            ...
            print_error("Power_License0_Utilization zero division")
            self.errcount += 1
            self.val = 0
...
512-bit instructions."""
    ...
    def compute(self, EV):
        try:
            ...
            print_error("Power_License1_Utilization zero division")
            self.errcount += 1
            self.val = 0
...
This includes high current AVX 512-bit instructions."""
    ...
    def compute(self, EV):
        try:
            ...
            print_error("Power_License2_Utilization zero division")
            self.errcount += 1
            self.val = 0
...
class Setup:
    def __init__(self, r):
        ...
        n = Metric_ITLB_Misses_Legacy() ; r.metric(n) ; o["ITLB_Misses_Legacy"] = n
