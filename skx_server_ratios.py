# -*- coding: latin-1 -*-
#
# auto generated TopDown/TMA 5.1-full description for Intel Xeon Scalable Processors (code named Skylake Server)
...
def Br_DoI_Jumps(self, EV, level):
    return EV("BR_INST_RETIRED.NEAR_TAKEN", level) - (EV("BR_INST_RETIRED.COND", level) - EV("BR_INST_RETIRED.NOT_TAKEN", level)) - 2 * EV("BR_INST_RETIRED.NEAR_CALL", level)
...
def Serialize_Core(self, EV, level):
    return self.Core_Bound.compute(EV) * (self.Serializing_Operation.compute(EV) + self.Core_Bound.compute(EV) * EV("RS_EVENTS.EMPTY_CYCLES", level) / CLKS(self, EV, level) * self.Ports_Utilized_0.compute(EV)) / (self.Divider.compute(EV) + self.Serializing_Operation.compute(EV) + self.Ports_Utilization.compute(EV))
...
def Assist_Frontend(self, EV, level):
    return Assist(self, EV, level) * self.Fetch_Latency.compute(EV) * (self.MS_Switches.compute(EV) + self.Branch_Resteers.compute(EV) * (self.Clears_Resteers.compute(EV) + self.Mispredicts_Resteers.compute(EV) * Umisp(self, EV, level)) / (self.Unknown_Branches.compute(EV) + self.Mispredicts_Resteers.compute(EV) + self.Clears_Resteers.compute(EV))) / (self.DSB_Switches.compute(EV) + self.LCP.compute(EV) + self.ITLB_Misses.compute(EV) + self.Branch_Resteers.compute(EV) + self.ICache_Misses.compute(EV) + self.MS_Switches.compute(EV))
...
def FP_Arith_Vector(self, EV, level):
    return EV("FP_ARITH_INST_RETIRED.128B_PACKED_DOUBLE:u0xfc", level)
...
def Light_Ops_Sum(self, EV, level):
    return self.FP_Arith.compute(EV) + self.Memory_Operations.compute(EV) + self.Fused_Instructions.compute(EV) + self.Non_Fused_Branches.compute(EV)
...
def LOAD_XSNP_HITM(self, EV, level):
    return EV("MEM_LOAD_L3_HIT_RETIRED.XSNP_HITM", level) * True_XSNP_HitM_Fraction(self, EV, level)
...
def OCR_all_rfo_l3_hit_snoop_hitm(self, EV, level):
    return EV("OFFCORE_RESPONSE.DEMAND_RFO.L3_HIT.HITM_OTHER_CORE", level) + EV("OFFCORE_RESPONSE.PF_L2_RFO.L3_HIT.HITM_OTHER_CORE", level)

def OCR_all_rfo_l3_miss_remote_hitm(self, EV, level):
    return EV("OFFCORE_RESPONSE.DEMAND_RFO.L3_MISS.REMOTE_HITM", level) + EV("OFFCORE_RESPONSE.PF_L2_RFO.L3_MISS.REMOTE_HITM", level)
...
def True_XSNP_HitM_Fraction(self, EV, level):
    return EV("OFFCORE_RESPONSE.DEMAND_DATA_RD.L3_HIT.HITM_OTHER_CORE", level) / (EV("OFFCORE_RESPONSE.DEMAND_DATA_RD.L3_HIT.HITM_OTHER_CORE", level) + EV("OFFCORE_RESPONSE.DEMAND_DATA_RD.L3_HIT.SNOOP_HIT_WITH_FWD", level))
...
def Mispredictions(self, EV, level):
    val = 100 *(1 - Umisp(self, EV, level)) * (self.Branch_Mispredicts.compute(EV) + self.Fetch_Latency.compute(EV) * self.Mispredicts_Resteers.compute(EV) / (self.DSB_Switches.compute(EV) + self.LCP.compute(EV) + self.ITLB_Misses.compute(EV) + self.Branch_Resteers.compute(EV) + self.ICache_Misses.compute(EV) + self.MS_Switches.compute(EV)))
    self.thresh = (val > 20)
    return val
...
def Big_Code(self, EV, level):
    val = 100 * self.Fetch_Latency.compute(EV) * (self.ITLB_Misses.compute(EV) + self.ICache_Misses.compute(EV) + self.Unknown_Branches.compute(EV)) / (self.DSB_Switches.compute(EV) + self.LCP.compute(EV) + self.ITLB_Misses.compute(EV) + self.Branch_Resteers.compute(EV) + self.ICache_Misses.compute(EV) + self.MS_Switches.compute(EV))
    self.thresh = (val > 20)
    return val
...
def Data_Cache_Memory_Bandwidth(self, EV, level):
    val = 100 *((self.Memory_Bound.compute(EV) * (self.DRAM_Bound.compute(EV) / (self.L2_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.Store_Bound.compute(EV))) * (self.MEM_Bandwidth.compute(EV) / (self.MEM_Latency.compute(EV) + self.MEM_Bandwidth.compute(EV)))) + (self.Memory_Bound.compute(EV) * (self.L3_Bound.compute(EV) / (self.L2_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.Store_Bound.compute(EV))) * (self.SQ_Full.compute(EV) / (self.Data_Sharing.compute(EV) + self.SQ_Full.compute(EV) + self.Contested_Accesses.compute(EV) + self.L3_Hit_Latency.compute(EV)))) + (self.Memory_Bound.compute(EV) * (self.L1_Bound.compute(EV) / (self.L2_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.Store_Bound.compute(EV))) * (self.FB_Full.compute(EV) / (self.G4K_Aliasing.compute(EV) + self.Store_Fwd_Blk.compute(EV) + self.FB_Full.compute(EV) + self.Split_Loads.compute(EV) + self.L1_Latency_Dependency.compute(EV) + self.Lock_Latency.compute(EV) + self.DTLB_Load.compute(EV)))))
    self.thresh = (val > 20)
    return val

# Total pipeline cost of external Memory- or Cache-Latency related bottlenecks
def Data_Cache_Memory_Latency(self, EV, level):
    val = 100 *((self.Memory_Bound.compute(EV) * (self.DRAM_Bound.compute(EV) / (self.L2_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.Store_Bound.compute(EV))) * (self.MEM_Latency.compute(EV) / (self.MEM_Latency.compute(EV) + self.MEM_Bandwidth.compute(EV)))) + (self.Memory_Bound.compute(EV) * (self.L3_Bound.compute(EV) / (self.L2_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.Store_Bound.compute(EV))) * (self.L3_Hit_Latency.compute(EV) / (self.Data_Sharing.compute(EV) + self.SQ_Full.compute(EV) + self.Contested_Accesses.compute(EV) + self.L3_Hit_Latency.compute(EV)))) + (self.Memory_Bound.compute(EV) * self.L2_Bound.compute(EV) / (self.L2_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.Store_Bound.compute(EV))) + (self.Memory_Bound.compute(EV) * (self.L1_Bound.compute(EV) / (self.L2_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.Store_Bound.compute(EV))) * (self.L1_Latency_Dependency.compute(EV) / (self.G4K_Aliasing.compute(EV) + self.Store_Fwd_Blk.compute(EV) + self.FB_Full.compute(EV) + self.Split_Loads.compute(EV) + self.L1_Latency_Dependency.compute(EV) + self.Lock_Latency.compute(EV) + self.DTLB_Load.compute(EV)))) + (self.Memory_Bound.compute(EV) * (self.L1_Bound.compute(EV) / (self.L2_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.Store_Bound.compute(EV))) * (self.Lock_Latency.compute(EV) / (self.G4K_Aliasing.compute(EV) + self.Store_Fwd_Blk.compute(EV) + self.FB_Full.compute(EV) + self.Split_Loads.compute(EV) + self.L1_Latency_Dependency.compute(EV) + self.Lock_Latency.compute(EV) + self.DTLB_Load.compute(EV)))) + (self.Memory_Bound.compute(EV) * (self.L1_Bound.compute(EV) / (self.L2_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.Store_Bound.compute(EV))) * (self.Split_Loads.compute(EV) / (self.G4K_Aliasing.compute(EV) + self.Store_Fwd_Blk.compute(EV) + self.FB_Full.compute(EV) + self.Split_Loads.compute(EV) + self.L1_Latency_Dependency.compute(EV) + self.Lock_Latency.compute(EV) + self.DTLB_Load.compute(EV)))) + (self.Memory_Bound.compute(EV) * (self.Store_Bound.compute(EV) / (self.L2_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.Store_Bound.compute(EV))) * (self.Split_Stores.compute(EV) / (self.DTLB_Store.compute(EV) + self.Split_Stores.compute(EV) + self.False_Sharing.compute(EV) + self.Store_Latency.compute(EV)))) + (self.Memory_Bound.compute(EV) * (self.Store_Bound.compute(EV) / (self.L2_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.Store_Bound.compute(EV))) * (self.Store_Latency.compute(EV) / (self.DTLB_Store.compute(EV) + self.Split_Stores.compute(EV) + self.False_Sharing.compute(EV) + self.Store_Latency.compute(EV)))))
    self.thresh = (val > 20)
    return val

# Total pipeline cost of Memory Address Translation related bottlenecks (data-side TLBs)
def Memory_Data_TLBs(self, EV, level):
    val = 100 *(self.Memory_Bound.compute(EV) * (self.L1_Bound.compute(EV) / max(self.Memory_Bound.compute(EV) , (self.L2_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.Store_Bound.compute(EV)))) * (self.DTLB_Load.compute(EV) / max(self.L1_Bound.compute(EV) , (self.G4K_Aliasing.compute(EV) + self.Store_Fwd_Blk.compute(EV) + self.FB_Full.compute(EV) + self.Split_Loads.compute(EV) + self.L1_Latency_Dependency.compute(EV) + self.Lock_Latency.compute(EV) + self.DTLB_Load.compute(EV)))) + (self.Memory_Bound.compute(EV) * (self.Store_Bound.compute(EV) / (self.L2_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.Store_Bound.compute(EV))) * (self.DTLB_Store.compute(EV) / (self.DTLB_Store.compute(EV) + self.Split_Stores.compute(EV) + self.False_Sharing.compute(EV) + self.Store_Latency.compute(EV)))))
    self.thresh = (val > 20)
    return val
...
def Memory_Synchronization(self, EV, level):
    val = 100 *(self.Memory_Bound.compute(EV) * ((self.DRAM_Bound.compute(EV) / (self.L2_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.Store_Bound.compute(EV))) * (self.MEM_Latency.compute(EV) / (self.MEM_Latency.compute(EV) + self.MEM_Bandwidth.compute(EV))) * self.Remote_Cache.compute(EV) / (self.Remote_Cache.compute(EV) + self.Local_MEM.compute(EV) + self.Remote_MEM.compute(EV)) + (self.L3_Bound.compute(EV) / (self.L2_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.Store_Bound.compute(EV))) * (self.Contested_Accesses.compute(EV) + self.Data_Sharing.compute(EV)) / (self.Data_Sharing.compute(EV) + self.SQ_Full.compute(EV) + self.Contested_Accesses.compute(EV) + self.L3_Hit_Latency.compute(EV)) + (self.Store_Bound.compute(EV) / (self.L2_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.Store_Bound.compute(EV))) * self.False_Sharing.compute(EV) / ((self.DTLB_Store.compute(EV) + self.Split_Stores.compute(EV) + self.False_Sharing.compute(EV) + self.Store_Latency.compute(EV)) - self.Store_Latency.compute(EV))) + self.Machine_Clears.compute(EV) * (1 - self.Other_Nukes.compute(EV) / (self.Other_Nukes.compute(EV)))) if DS else 100 *(self.Memory_Bound.compute(EV) * ((self.L3_Bound.compute(EV) / (self.L2_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.Store_Bound.compute(EV))) * (self.Contested_Accesses.compute(EV) + self.Data_Sharing.compute(EV)) / (self.Data_Sharing.compute(EV) + self.SQ_Full.compute(EV) + self.Contested_Accesses.compute(EV) + self.L3_Hit_Latency.compute(EV)) + (self.Store_Bound.compute(EV) / (self.L2_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.Store_Bound.compute(EV))) * self.False_Sharing.compute(EV) / ((self.DTLB_Store.compute(EV) + self.Split_Stores.compute(EV) + self.False_Sharing.compute(EV) + self.Store_Latency.compute(EV)) - self.Store_Latency.compute(EV))) + self.Machine_Clears.compute(EV) * (1 - self.Other_Nukes.compute(EV) / (self.Other_Nukes.compute(EV))))
    self.thresh = (val > 10)
    return val
...
def IpAssist(self, EV, level):
    val = EV("INST_RETIRED.ANY", level) / (EV("FP_ASSIST.ANY", level) + EV("OTHER_ASSISTS.ANY", level))
...
def Fetch_MITE(self, EV, level):
    return EV("IDQ.MITE_UOPS", level) / EV("IDQ.MITE_CYCLES", level)
...
def DSB_Misses(self, EV, level):
    val = 100 *(self.Fetch_Latency.compute(EV) * self.DSB_Switches.compute(EV) / (self.DSB_Switches.compute(EV) + self.LCP.compute(EV) + self.ITLB_Misses.compute(EV) + self.Branch_Resteers.compute(EV) + self.ICache_Misses.compute(EV) + self.MS_Switches.compute(EV)) + self.Fetch_Bandwidth.compute(EV) * self.MITE.compute(EV) / (self.MITE.compute(EV) + self.DSB.compute(EV)))
    self.thresh = (val > 10)
    return val

# Total pipeline cost of DSB (uop cache) hits - subset of the Instruction_Fetch_BW Bottleneck.
def DSB_Bandwidth(self, EV, level):
    val = 100 *(self.Frontend_Bound.compute(EV) * (self.Fetch_Bandwidth.compute(EV) / (self.Fetch_Latency.compute(EV) + self.Fetch_Bandwidth.compute(EV))) * (self.DSB.compute(EV) / (self.MITE.compute(EV) + self.DSB.compute(EV))))
    self.thresh = (val > 10)
    return val
...
def IC_Misses(self, EV, level):
    val = 100 *(self.Fetch_Latency.compute(EV) * self.ICache_Misses.compute(EV) / (self.DSB_Switches.compute(EV) + self.LCP.compute(EV) + self.ITLB_Misses.compute(EV) + self.Branch_Resteers.compute(EV) + self.ICache_Misses.compute(EV) + self.MS_Switches.compute(EV)))
    self.thresh = (val > 5)
    return val
...
class Branch_Resteers:
    ...
    def compute(self, EV):
        try:
            self.val = EV("INT_MISC.CLEAR_RESTEER_CYCLES", 3) / CLKS(self, EV, 3) + BAClear_Cost * EV("BACLEARS.ANY", 3) / CLKS(self, EV, 3)
            self.thresh = (self.val > 0.05) and self.parent.thresh
        except ZeroDivisionError:
...
class False_Sharing:
    ...
    def compute(self, EV):
        try:
            self.val = (Mem_Remote_HitM_Cost(self, EV, 4) * OCR_all_rfo_l3_miss_remote_hitm(self, EV, 4) + Mem_XSNP_HitM_Cost(self, EV, 4) * OCR_all_rfo_l3_hit_snoop_hitm(self, EV, 4)) / CLKS(self, EV, 4) if DS else Mem_XSNP_HitM_Cost(self, EV, 4) * OCR_all_rfo_l3_hit_snoop_hitm(self, EV, 4) / CLKS(self, EV, 4)
            self.thresh = (self.val > 0.05) and self.parent.thresh
        except ZeroDivisionError:
...
class Serializing_Operation:
    ...
    def compute(self, EV):
        try:
            self.val = EV("PARTIAL_RAT_STALLS.SCOREBOARD", 3) / CLKS(self, EV, 3)
            self.thresh = (self.val > 0.1) and self.parent.thresh
        except ZeroDivisionError:
...
class FP_Assists:
    ...
    def compute(self, EV):
        try:
            self.val = 34 * EV("FP_ASSIST.ANY", 5) / SLOTS(self, EV, 5)
            self.thresh = (self.val > 0.1)
        except ZeroDivisionError:
...
class Setup:
    def __init__(self, r):
        ...
        o["Data_Cache_Memory_Bandwidth"].Frontend_Bound = o["Frontend_Bound"]
        o["Data_Cache_Memory_Bandwidth"].G4K_Aliasing = o["G4K_Aliasing"]
        o["Data_Cache_Memory_Bandwidth"].Store_Fwd_Blk = o["Store_Fwd_Blk"]
        o["Data_Cache_Memory_Bandwidth"].MEM_Latency = o["MEM_Latency"]
        o["Data_Cache_Memory_Bandwidth"].Retiring = o["Retiring"]
        ...
        o["Data_Cache_Memory_Bandwidth"].Backend_Bound = o["Backend_Bound"]
        ...
        o["Data_Cache_Memory_Latency"].Frontend_Bound = o["Frontend_Bound"]
        o["Data_Cache_Memory_Latency"].G4K_Aliasing = o["G4K_Aliasing"]
        ...
        o["Data_Cache_Memory_Latency"].Retiring = o["Retiring"]
        ...
        o["Data_Cache_Memory_Latency"].Backend_Bound = o["Backend_Bound"]
        ...
        o["Memory_Synchronization"].MEM_Latency = o["MEM_Latency"]
        ...
        o["Memory_Synchronization"].Remote_MEM = o["Remote_MEM"]
        ...
        o["Memory_Synchronization"].MEM_Bandwidth = o["MEM_Bandwidth"]
        o["Memory_Synchronization"].Remote_Cache = o["Remote_Cache"]
        ...
        o["Other_Bottlenecks"].Remote_Cache = o["Remote_Cache"]
        ...
        o["Other_Bottlenecks"].Local_MEM = o["Local_MEM"]
        ...
        o["Other_Bottlenecks"].Remote_MEM = o["Remote_MEM"]
