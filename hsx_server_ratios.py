...
#
# auto generated TopDown/TMAM 3.02r description for Intel Xeon E5 v3 (code Named Haswell EP)
...
def Cycles_False_Sharing_Client(self, EV, level):
    return Mem_XSNP_HitM_Cost *(EV("MEM_LOAD_UOPS_L3_HIT_RETIRED.XSNP_HITM", level) + EV("OFFCORE_RESPONSE:request=DEMAND_RFO:response=LLC_HIT.HITM_OTHER_CORE", level))
...
and increase performance."""
    ...
    def compute(self, EV):
        try:
            self.val = Mem_Local_DRAM_Cost * EV("MEM_LOAD_UOPS_L3_MISS_RETIRED.LOCAL_DRAM", 5) / CLKS(self, EV, 5 )
            self.thresh = (self.val > 0.1) and self.parent.thresh
        except ZeroDivisionError:
...
non-optimal NUMA allocations."""
    ...
    def compute(self, EV):
        try:
            self.val = Mem_Remote_DRAM_Cost * EV("MEM_LOAD_UOPS_L3_MISS_RETIRED.REMOTE_DRAM", 5) / CLKS(self, EV, 5 )
            self.thresh = (self.val > 0.1) and self.parent.thresh
        except ZeroDivisionError:
...
often due to non-optimal NUMA allocations."""
    ...
    def compute(self, EV):
        try:
            self.val = (Mem_Remote_HitM_Cost * EV("MEM_LOAD_UOPS_L3_MISS_RETIRED.REMOTE_HITM", 5) + Mem_Remote_Fwd_Cost * EV("MEM_LOAD_UOPS_L3_MISS_RETIRED.REMOTE_FWD", 5)) / CLKS(self, EV, 5 )
            self.thresh = (self.val > 0.1) and self.parent.thresh
        except ZeroDivisionError:
...
padding to make threads access different lines."""
    ...
    def compute(self, EV):
        try:
            self.val = (Mem_Remote_HitM_Cost * EV("OFFCORE_RESPONSE:request=DEMAND_RFO:response=LLC_MISS.REMOTE_HITM", 4) + Cycles_False_Sharing_Client(self, EV, 4)) / CLKS(self, EV, 4 )
            self.thresh = (self.val > 0.2) and self.parent.thresh
        except ZeroDivisionError:
