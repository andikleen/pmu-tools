# -*- coding: latin-1 -*-
#
# auto generated TopDown/TMA 4.5-full-perf description for Intel Xeon Scalable Processors 4th gen (code name Sapphire Rapids)
...
def Core_Bound_Cycles(self, EV, level):
    return EV("EXE_ACTIVITY.3_PORTS_UTIL:u0x80", level) + self.Serializing_Operation.compute(EV) * (EV("CYCLE_ACTIVITY.STALLS_TOTAL", level) - EV("EXE_ACTIVITY.BOUND_ON_LOADS", level)) + Few_Uops_Executed_Threshold(self, EV, level)
...
def FLOP_Count(self, EV, level):
    EV("FP_ARITH_INST_RETIRED.256B_PACKED_SINGLE:u0x60", level)
    ...
    EV("FP_ARITH_INST_RETIRED.SCALAR_SINGLE:u0x03", level)
    ...
    EV("FP_ARITH_INST_RETIRED.128B_PACKED_SINGLE:u0x18", level)
    return (1 *(EV("FP_ARITH_INST_RETIRED.SCALAR_SINGLE:u0x03", level) + EV("FP_ARITH_INST_RETIRED2.SCALAR_HALF", level)) + 2 *(EV("FP_ARITH_INST_RETIRED.128B_PACKED_DOUBLE", level) + EV("FP_ARITH_INST_RETIRED2.COMPLEX_SCALAR_HALF", level)) + 4 * EV("FP_ARITH_INST_RETIRED.128B_PACKED_SINGLE:u0x18", level) + 8 *(EV("FP_ARITH_INST_RETIRED2.128B_PACKED_HALF", level) + EV("FP_ARITH_INST_RETIRED.256B_PACKED_SINGLE:u0x60", level)) + 16 *(EV("FP_ARITH_INST_RETIRED2.256B_PACKED_HALF", level) + EV("FP_ARITH_INST_RETIRED.512B_PACKED_SINGLE", level)) + 32 * EV("FP_ARITH_INST_RETIRED2.512B_PACKED_HALF", level) + 4 * EV("AMX_OPS_RETIRED.BF16", level)) if FP16(self, EV, level) else(1 * EV("FP_ARITH_INST_RETIRED.SCALAR_SINGLE:u0x03", level) + 2 * EV("FP_ARITH_INST_RETIRED.128B_PACKED_DOUBLE", level) + 4 * EV("FP_ARITH_INST_RETIRED.128B_PACKED_SINGLE:u0x18", level) + 8 * EV("FP_ARITH_INST_RETIRED.256B_PACKED_SINGLE:u0x60", level) + 16 * EV("FP_ARITH_INST_RETIRED.512B_PACKED_SINGLE", level))

# Floating Point computational (arithmetic) Operations Count
def FP_Arith_Scalar(self, EV, level):
    return EV("FP_ARITH_INST_RETIRED.SCALAR_SINGLE:u0x03", level) + EV("FP_ARITH_INST_RETIRED2.SCALAR", level)
...
def Mem_XSNP_Hit_Cost(self, EV, level):
    return 79.5 * Average_Frequency(self, EV, level)

def Mem_XSNP_None_Cost(self, EV, level):
    return 37 * Average_Frequency(self, EV, level)

def Mem_Local_DRAM_Cost(self, EV, level):
    return 108 * Average_Frequency(self, EV, level)

def Mem_Remote_DRAM_Cost(self, EV, level):
    return 186 * Average_Frequency(self, EV, level)

def Mem_Remote_HitM_Cost(self, EV, level):
    return 172.5 * Average_Frequency(self, EV, level)
...
def IpAssist(self, EV, level):
    val = EV("INST_RETIRED.ANY", level) / EV("ASSISTS.ANY:u0x1B", level)
...
def TIOPS(self, EV, level):
    return (8 * EV("AMX_OPS_RETIRED.INT8", level) / 1e12) / Time(self, EV, level)
...
class L3_Hit_Latency:
    ...
    def compute(self, EV):
        try:
            self.val = (Mem_XSNP_None_Cost(self, EV, 4) - Mem_L2_Hit_Cost(self, EV, 4)) * LOAD_L3_HIT(self, EV, 4) * FB_Factor(self, EV, 4) / CLKS(self, EV, 4)
            self.val = min(self.val, 1.0)
            self.thresh = (self.val > 0.1) and self.parent.thresh
        except ZeroDivisionError:
...
class Ports_Utilized_0:
    ...
    def compute(self, EV):
        try:
            self.val = EV("EXE_ACTIVITY.3_PORTS_UTIL:u0x80", 4) / CLKS(self, EV, 4) + self.Serializing_Operation.compute(EV) * (EV("CYCLE_ACTIVITY.STALLS_TOTAL", 4) - EV("EXE_ACTIVITY.BOUND_ON_LOADS", 4)) / CLKS(self, EV, 4)
            self.thresh = (self.val > 0.2) and self.parent.thresh
        except ZeroDivisionError:
...
class Int_Operations:
    ...
    def compute(self, EV):
        try:
            self.val = self.Int_Vector_128b.compute(EV) + self.Int_Vector_256b.compute(EV) + self.Shuffles.compute(EV) + self.Int_AMX.compute(EV)
            self.thresh = (self.val > 0.1) and self.parent.thresh
        except ZeroDivisionError:
...
floating point values (so-called Denormals).. Consider
enabling the DAZ (Denormals Are Zero) and/or FTZ (Flush To
Zero) options in your compiler to flush denormals to zero.
...
class Setup:
    def __init__(self, r):
        ...
        o["IpTB"].sibling = (o["DSB_Switches"], o["LCP"], o["Fetch_Bandwidth"],)
        o["DSB_Coverage"].sibling = (o["DSB_Switches"], o["LCP"], o["Fetch_Bandwidth"],)
        o["DSB_Misses"].sibling = (o["DSB_Switches"], o["LCP"], o["Fetch_Bandwidth"],)
