...
#
# auto generated TopDown/TMAM 3.31-full description for Intel 4rd gen Core (code named Haswell)
...
lines."""
    level = 4
    htoff = False
    sample = ['MEM_LOAD_UOPS_L3_HIT_RETIRED.XSNP_HITM:pp', 'OFFCORE_RESPONSE:request=DEMAND_RFO:response=L3_HIT.HITM_OTHER_CORE']
    errcount = 0
    sibling = None
    server = False
    metricgroup = ['Data_Sharing']
    def compute(self, EV):
        try:
            self.val = Mem_XSNP_HitM_Cost * EV("OFFCORE_RESPONSE:request=DEMAND_RFO:response=L3_HIT.HITM_OTHER_CORE", 4) / CLKS(self, EV, 4 )
            self.thresh = (self.val > 0.1) & self.parent.thresh
        except ZeroDivisionError:
