# -*- coding: latin-1 -*-
#
# auto generated TopDown/TMA 5.1-full description for Intel Xeon Scalable Processors 3rd gen (code name Icelake Server)
...
def Assist_Frontend(self, EV, level):
    return Assist(self, EV, level) * (self.Fetch_Latency.compute(EV) * (self.MS_Switches.compute(EV) + self.Branch_Resteers.compute(EV) * (self.Clears_Resteers.compute(EV) + Umisp(self, EV, level) * self.Mispredicts_Resteers.compute(EV)) / (self.Unknown_Branches.compute(EV) + self.Mispredicts_Resteers.compute(EV) + self.Clears_Resteers.compute(EV))) / (self.ITLB_Misses.compute(EV) + self.MS_Switches.compute(EV) + self.DSB_Switches.compute(EV) + self.Branch_Resteers.compute(EV) + self.LCP.compute(EV) + self.ICache_Misses.compute(EV)) + self.MS.compute(EV))
...
def LOAD_LCL_PMM(self, EV, level):
    ...
    return EV("MEM_LOAD_RETIRED.LOCAL_PMM", level) * (1 + FBHit_per_L1Miss(self, EV, level)) if PMM_App_Direct else 0
...
def LOAD_RMT_PMM(self, EV, level):
    ...
    return EV("MEM_LOAD_L3_MISS_RETIRED.REMOTE_PMM", level) * (1 + FBHit_per_L1Miss(self, EV, level)) if PMM_App_Direct else 0
...
def Num_CPUs(self, EV, level):
    return num_cores * num_sockets * num_threads if num_cores else 160 /(2 - smt_enabled )
...
def Mispredictions(self, EV, level):
    val = 100 *(1 - Umisp(self, EV, level)) * (self.Branch_Mispredicts.compute(EV) + self.Fetch_Latency.compute(EV) * self.Mispredicts_Resteers.compute(EV) / (self.ITLB_Misses.compute(EV) + self.MS_Switches.compute(EV) + self.DSB_Switches.compute(EV) + self.Branch_Resteers.compute(EV) + self.LCP.compute(EV) + self.ICache_Misses.compute(EV)))
    self.thresh = (val > 20)
    return val
...
def Big_Code(self, EV, level):
    val = 100 * self.Fetch_Latency.compute(EV) * (self.ITLB_Misses.compute(EV) + self.ICache_Misses.compute(EV) + self.Unknown_Branches.compute(EV)) / (self.ITLB_Misses.compute(EV) + self.MS_Switches.compute(EV) + self.DSB_Switches.compute(EV) + self.Branch_Resteers.compute(EV) + self.LCP.compute(EV) + self.ICache_Misses.compute(EV))
    self.thresh = (val > 20)
    return val
...
def Instruction_Fetch_BW(self, EV, level):
    val = 100 *(self.Frontend_Bound.compute(EV) - (1 - Umisp(self, EV, level)) * self.Fetch_Latency.compute(EV) * self.Mispredicts_Resteers.compute(EV) / (self.ITLB_Misses.compute(EV) + self.MS_Switches.compute(EV) + self.DSB_Switches.compute(EV) + self.Branch_Resteers.compute(EV) + self.LCP.compute(EV) + self.ICache_Misses.compute(EV)) - Assist_Frontend(self, EV, level)) - Big_Code(self, EV, level)
    self.thresh = (val > 20)
    return val
...
def Data_Cache_Memory_Bandwidth(self, EV, level):
    val = 100 *((self.Memory_Bound.compute(EV) * (self.DRAM_Bound.compute(EV) / (self.Store_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.CXL_Mem_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.L2_Bound.compute(EV))) * (self.MEM_Bandwidth.compute(EV) / (self.MEM_Bandwidth.compute(EV) + self.MEM_Latency.compute(EV)))) + (self.Memory_Bound.compute(EV) * (self.L3_Bound.compute(EV) / (self.Store_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.CXL_Mem_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.L2_Bound.compute(EV))) * (self.SQ_Full.compute(EV) / (self.Contested_Accesses.compute(EV) + self.SQ_Full.compute(EV) + self.L3_Hit_Latency.compute(EV) + self.Data_Sharing.compute(EV)))) + (self.Memory_Bound.compute(EV) * (self.L1_Bound.compute(EV) / (self.Store_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.CXL_Mem_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.L2_Bound.compute(EV))) * (self.FB_Full.compute(EV) / (self.Store_Fwd_Blk.compute(EV) + self.Lock_Latency.compute(EV) + self.L1_Latency_Dependency.compute(EV) + self.DTLB_Load.compute(EV) + self.FB_Full.compute(EV) + self.Split_Loads.compute(EV) + self.G4K_Aliasing.compute(EV)))))
    self.thresh = (val > 20)
    return val

# Total pipeline cost of external Memory- or Cache-Latency related bottlenecks
def Data_Cache_Memory_Latency(self, EV, level):
    val = 100 *((self.Memory_Bound.compute(EV) * (self.DRAM_Bound.compute(EV) / (self.Store_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.CXL_Mem_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.L2_Bound.compute(EV))) * (self.MEM_Latency.compute(EV) / (self.MEM_Bandwidth.compute(EV) + self.MEM_Latency.compute(EV)))) + (self.Memory_Bound.compute(EV) * (self.L3_Bound.compute(EV) / (self.Store_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.CXL_Mem_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.L2_Bound.compute(EV))) * (self.L3_Hit_Latency.compute(EV) / (self.Contested_Accesses.compute(EV) + self.SQ_Full.compute(EV) + self.L3_Hit_Latency.compute(EV) + self.Data_Sharing.compute(EV)))) + (self.Memory_Bound.compute(EV) * self.L2_Bound.compute(EV) / (self.Store_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.CXL_Mem_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.L2_Bound.compute(EV))) + (self.Memory_Bound.compute(EV) * (self.L1_Bound.compute(EV) / (self.Store_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.CXL_Mem_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.L2_Bound.compute(EV))) * (self.L1_Latency_Dependency.compute(EV) / (self.Store_Fwd_Blk.compute(EV) + self.Lock_Latency.compute(EV) + self.L1_Latency_Dependency.compute(EV) + self.DTLB_Load.compute(EV) + self.FB_Full.compute(EV) + self.Split_Loads.compute(EV) + self.G4K_Aliasing.compute(EV)))) + (self.Memory_Bound.compute(EV) * (self.L1_Bound.compute(EV) / (self.Store_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.CXL_Mem_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.L2_Bound.compute(EV))) * (self.Lock_Latency.compute(EV) / (self.Store_Fwd_Blk.compute(EV) + self.Lock_Latency.compute(EV) + self.L1_Latency_Dependency.compute(EV) + self.DTLB_Load.compute(EV) + self.FB_Full.compute(EV) + self.Split_Loads.compute(EV) + self.G4K_Aliasing.compute(EV)))) + (self.Memory_Bound.compute(EV) * (self.L1_Bound.compute(EV) / (self.Store_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.CXL_Mem_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.L2_Bound.compute(EV))) * (self.Split_Loads.compute(EV) / (self.Store_Fwd_Blk.compute(EV) + self.Lock_Latency.compute(EV) + self.L1_Latency_Dependency.compute(EV) + self.DTLB_Load.compute(EV) + self.FB_Full.compute(EV) + self.Split_Loads.compute(EV) + self.G4K_Aliasing.compute(EV)))) + (self.Memory_Bound.compute(EV) * (self.Store_Bound.compute(EV) / (self.Store_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.CXL_Mem_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.L2_Bound.compute(EV))) * (self.Split_Stores.compute(EV) / (self.DTLB_Store.compute(EV) + self.Store_Latency.compute(EV) + self.Streaming_Stores.compute(EV) + self.Split_Stores.compute(EV) + self.False_Sharing.compute(EV)))) + (self.Memory_Bound.compute(EV) * (self.Store_Bound.compute(EV) / (self.Store_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.CXL_Mem_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.L2_Bound.compute(EV))) * (self.Store_Latency.compute(EV) / (self.DTLB_Store.compute(EV) + self.Store_Latency.compute(EV) + self.Streaming_Stores.compute(EV) + self.Split_Stores.compute(EV) + self.False_Sharing.compute(EV)))))
    self.thresh = (val > 20)
    return val

# Total pipeline cost of Memory Address Translation related bottlenecks (data-side TLBs)
def Memory_Data_TLBs(self, EV, level):
    val = 100 *(self.Memory_Bound.compute(EV) * (self.L1_Bound.compute(EV) / max(self.Memory_Bound.compute(EV) , (self.Store_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.CXL_Mem_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.L2_Bound.compute(EV)))) * (self.DTLB_Load.compute(EV) / max(self.L1_Bound.compute(EV) , (self.Store_Fwd_Blk.compute(EV) + self.Lock_Latency.compute(EV) + self.L1_Latency_Dependency.compute(EV) + self.DTLB_Load.compute(EV) + self.FB_Full.compute(EV) + self.Split_Loads.compute(EV) + self.G4K_Aliasing.compute(EV)))) + (self.Memory_Bound.compute(EV) * (self.Store_Bound.compute(EV) / (self.Store_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.CXL_Mem_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.L2_Bound.compute(EV))) * (self.DTLB_Store.compute(EV) / (self.DTLB_Store.compute(EV) + self.Store_Latency.compute(EV) + self.Streaming_Stores.compute(EV) + self.Split_Stores.compute(EV) + self.False_Sharing.compute(EV)))))
    self.thresh = (val > 20)
    return val
...
def Memory_Synchronization(self, EV, level):
    val = 100 *(self.Memory_Bound.compute(EV) * ((self.L3_Bound.compute(EV) / (self.Store_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.CXL_Mem_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.L2_Bound.compute(EV))) * (self.Contested_Accesses.compute(EV) + self.Data_Sharing.compute(EV)) / (self.Contested_Accesses.compute(EV) + self.SQ_Full.compute(EV) + self.L3_Hit_Latency.compute(EV) + self.Data_Sharing.compute(EV)) + (self.Store_Bound.compute(EV) / (self.Store_Bound.compute(EV) + self.L3_Bound.compute(EV) + self.CXL_Mem_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.L2_Bound.compute(EV))) * self.False_Sharing.compute(EV) / ((self.DTLB_Store.compute(EV) + self.Store_Latency.compute(EV) + self.Streaming_Stores.compute(EV) + self.Split_Stores.compute(EV) + self.False_Sharing.compute(EV)) - self.Store_Latency.compute(EV))) + self.Machine_Clears.compute(EV) * (1 - self.Other_Nukes.compute(EV) / (self.Other_Nukes.compute(EV))))
    self.thresh = (val > 10)
    return val
...
def DSB_Misses(self, EV, level):
    val = 100 *(self.Fetch_Latency.compute(EV) * self.DSB_Switches.compute(EV) / (self.ITLB_Misses.compute(EV) + self.MS_Switches.compute(EV) + self.DSB_Switches.compute(EV) + self.Branch_Resteers.compute(EV) + self.LCP.compute(EV) + self.ICache_Misses.compute(EV)) + self.Fetch_Bandwidth.compute(EV) * self.MITE.compute(EV) / (self.DSB.compute(EV) + self.MITE.compute(EV) + self.MS.compute(EV)))
    self.thresh = (val > 10)
    return val

# Total pipeline cost of DSB (uop cache) hits - subset of the Instruction_Fetch_BW Bottleneck.
def DSB_Bandwidth(self, EV, level):
    val = 100 *(self.Frontend_Bound.compute(EV) * (self.Fetch_Bandwidth.compute(EV) / (self.Fetch_Bandwidth.compute(EV) + self.Fetch_Latency.compute(EV))) * (self.DSB.compute(EV) / (self.DSB.compute(EV) + self.MITE.compute(EV) + self.MS.compute(EV))))
    self.thresh = (val > 10)
    return val
...
def IC_Misses(self, EV, level):
    val = 100 *(self.Fetch_Latency.compute(EV) * self.ICache_Misses.compute(EV) / (self.ITLB_Misses.compute(EV) + self.MS_Switches.compute(EV) + self.DSB_Switches.compute(EV) + self.Branch_Resteers.compute(EV) + self.LCP.compute(EV) + self.ICache_Misses.compute(EV)))
    self.thresh = (val > 5)
    return val
...
class MS:
    ...
    def compute(self, EV):
        try:
            self.val = EV("IDQ.MS_UOPS:c1", 3) / CORE_CLKS(self, EV, 3) / 3.3
            self.thresh = (self.val > 0.05) and self.parent.thresh
        except ZeroDivisionError:
...
class CXL_Mem_Bound:
    ...
    def compute(self, EV):
        try:
            ...
            handle_error(self, "CXL_Mem_Bound zero division")
        return self.val
    desc = """
This metric roughly estimates (based on idle latencies) how
often the CPU was stalled on accesses to external CXL Memory
by loads (e.g. 3D-Xpoint (Crystal Ridge, a.k.a. IXP) memory,
PMM - Persistent Memory Module [from CLX to SPR] or any
other CXL Type3 Memory [EMR onwards]). . Consider moving
data-structure from AEP to DDR memory for better
latency/bandwidth."""
...
class FP_Divider:
    ...
    def compute(self, EV):
        try:
            self.val = EV("ARITH.FP_DIVIDER_ACTIVE", 4) / CLKS(self, EV, 4)
            self.thresh = (self.val > 0.2) and self.parent.thresh
        except ZeroDivisionError:
...
class Slow_Pause:
    ...
    def compute(self, EV):
        try:
            self.val = 37 * EV("MISC_RETIRED.PAUSE_INST", 4) / CLKS(self, EV, 4)
            self.thresh = (self.val > 0.05) and self.parent.thresh
        except ZeroDivisionError:
...
class Setup:
    def __init__(self, r):
        ...
        n = CXL_Mem_Bound() ; r.run(n) ; o["CXL_Mem_Bound"] = n
        ...
        o["CXL_Mem_Bound"].parent = o["Memory_Bound"]
        ...
        o["CXL_Mem_Bound"].L2_Bound = o["L2_Bound"]
        ...
        o["Data_Cache_Memory_Bandwidth"].CXL_Mem_Bound = o["CXL_Mem_Bound"]
        ...
        o["Data_Cache_Memory_Latency"].CXL_Mem_Bound = o["CXL_Mem_Bound"]
        ...
        o["Memory_Data_TLBs"].CXL_Mem_Bound = o["CXL_Mem_Bound"]
        ...
        o["Memory_Synchronization"].CXL_Mem_Bound = o["CXL_Mem_Bound"]
        ...
        o["Other_Bottlenecks"].CXL_Mem_Bound = o["CXL_Mem_Bound"]
