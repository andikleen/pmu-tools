# -*- coding: latin-1 -*-
#
# auto generated TopDown/TMA 5.1-full description for Intel Xeon Scalable Processors 4th gen MAX (code name Sapphire Rapids)
...
Energy_Unit = 61
Errata_Whitelist = "ADL038;ADL066"
Memory = 2
...
def Assist_Frontend(self, EV, level):
    return (1 - EV("INST_RETIRED.REP_ITERATION", level) / EV("UOPS_RETIRED.MS:c1", level)) * (self.Fetch_Latency.compute(EV) * (self.MS_Switches.compute(EV) + self.Branch_Resteers.compute(EV) * (self.Clears_Resteers.compute(EV) + self.Mispredicts_Resteers.compute(EV) * self.Other_Mispredicts.compute(EV) / self.Branch_Mispredicts.compute(EV)) / (self.Mispredicts_Resteers.compute(EV) + self.Clears_Resteers.compute(EV) + self.Unknown_Branches.compute(EV))) / (self.ICache_Misses.compute(EV) + self.MS_Switches.compute(EV) + self.DSB_Switches.compute(EV) + self.LCP.compute(EV) + self.Branch_Resteers.compute(EV) + self.ITLB_Misses.compute(EV)) + self.MS.compute(EV))
...
def Mispredictions(self, EV, level):
    val = 100 *(1 - Umisp(self, EV, level)) * (self.Branch_Mispredicts.compute(EV) + self.Fetch_Latency.compute(EV) * self.Mispredicts_Resteers.compute(EV) / (self.ICache_Misses.compute(EV) + self.MS_Switches.compute(EV) + self.DSB_Switches.compute(EV) + self.LCP.compute(EV) + self.Branch_Resteers.compute(EV) + self.ITLB_Misses.compute(EV)))
    self.thresh = (val > 20)
    return val
...
def Big_Code(self, EV, level):
    val = 100 * self.Fetch_Latency.compute(EV) * (self.ITLB_Misses.compute(EV) + self.ICache_Misses.compute(EV) + self.Unknown_Branches.compute(EV)) / (self.ICache_Misses.compute(EV) + self.MS_Switches.compute(EV) + self.DSB_Switches.compute(EV) + self.LCP.compute(EV) + self.Branch_Resteers.compute(EV) + self.ITLB_Misses.compute(EV))
    self.thresh = (val > 20)
    return val
...
def Instruction_Fetch_BW(self, EV, level):
    val = 100 *(self.Frontend_Bound.compute(EV) - (1 - Umisp(self, EV, level)) * self.Fetch_Latency.compute(EV) * self.Mispredicts_Resteers.compute(EV) / (self.ICache_Misses.compute(EV) + self.MS_Switches.compute(EV) + self.DSB_Switches.compute(EV) + self.LCP.compute(EV) + self.Branch_Resteers.compute(EV) + self.ITLB_Misses.compute(EV)) - Assist_Frontend(self, EV, level)) - Big_Code(self, EV, level)
    self.thresh = (val > 20)
    return val
...
def Data_Cache_Memory_Bandwidth(self, EV, level):
    val = (100 *((self.Memory_Bound.compute(EV) * (self.DRAM_Bound.compute(EV) / (self.L3_Bound.compute(EV) + self.L2_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.HBM_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.Store_Bound.compute(EV))) * (self.MEM_Bandwidth.compute(EV) / (self.MEM_Latency.compute(EV) + self.MEM_Bandwidth.compute(EV)))) + (self.Memory_Bound.compute(EV) * self.HBM_Bound.compute(EV) / (self.L3_Bound.compute(EV) + self.L2_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.HBM_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.Store_Bound.compute(EV))) * self.MEM_Bandwidth.compute(EV) / (self.MEM_Latency.compute(EV) + self.MEM_Bandwidth.compute(EV)) + (self.Memory_Bound.compute(EV) * (self.L3_Bound.compute(EV) / (self.L3_Bound.compute(EV) + self.L2_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.HBM_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.Store_Bound.compute(EV))) * (self.SQ_Full.compute(EV) / (self.L3_Hit_Latency.compute(EV) + self.Contested_Accesses.compute(EV) + self.SQ_Full.compute(EV) + self.Data_Sharing.compute(EV)))) + (self.Memory_Bound.compute(EV) * (self.L1_Bound.compute(EV) / (self.L3_Bound.compute(EV) + self.L2_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.HBM_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.Store_Bound.compute(EV))) * (self.FB_Full.compute(EV) / (self.Store_Fwd_Blk.compute(EV) + self.Split_Loads.compute(EV) + self.DTLB_Load.compute(EV) + self.FB_Full.compute(EV) + self.L1_Latency_Dependency.compute(EV) + self.Lock_Latency.compute(EV))))))
    self.thresh = (val > 20)
    return val

# Total pipeline cost of external Memory- or Cache-Latency related bottlenecks
def Data_Cache_Memory_Latency(self, EV, level):
    val = 100 *((self.Memory_Bound.compute(EV) * (self.DRAM_Bound.compute(EV) / (self.L3_Bound.compute(EV) + self.L2_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.HBM_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.Store_Bound.compute(EV))) * (self.MEM_Latency.compute(EV) / (self.MEM_Latency.compute(EV) + self.MEM_Bandwidth.compute(EV)))) + (self.Memory_Bound.compute(EV) * self.HBM_Bound.compute(EV) / (self.L3_Bound.compute(EV) + self.L2_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.HBM_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.Store_Bound.compute(EV))) * self.MEM_Latency.compute(EV) / (self.MEM_Latency.compute(EV) + self.MEM_Bandwidth.compute(EV)) + (self.Memory_Bound.compute(EV) * (self.L3_Bound.compute(EV) / (self.L3_Bound.compute(EV) + self.L2_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.HBM_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.Store_Bound.compute(EV))) * (self.L3_Hit_Latency.compute(EV) / (self.L3_Hit_Latency.compute(EV) + self.Contested_Accesses.compute(EV) + self.SQ_Full.compute(EV) + self.Data_Sharing.compute(EV)))) + (self.Memory_Bound.compute(EV) * self.L2_Bound.compute(EV) / (self.L3_Bound.compute(EV) + self.L2_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.HBM_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.Store_Bound.compute(EV))) + (self.Memory_Bound.compute(EV) * (self.L1_Bound.compute(EV) / (self.L3_Bound.compute(EV) + self.L2_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.HBM_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.Store_Bound.compute(EV))) * (self.L1_Latency_Dependency.compute(EV) / (self.Store_Fwd_Blk.compute(EV) + self.Split_Loads.compute(EV) + self.DTLB_Load.compute(EV) + self.FB_Full.compute(EV) + self.L1_Latency_Dependency.compute(EV) + self.Lock_Latency.compute(EV)))) + (self.Memory_Bound.compute(EV) * (self.L1_Bound.compute(EV) / (self.L3_Bound.compute(EV) + self.L2_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.HBM_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.Store_Bound.compute(EV))) * (self.Lock_Latency.compute(EV) / (self.Store_Fwd_Blk.compute(EV) + self.Split_Loads.compute(EV) + self.DTLB_Load.compute(EV) + self.FB_Full.compute(EV) + self.L1_Latency_Dependency.compute(EV) + self.Lock_Latency.compute(EV)))) + (self.Memory_Bound.compute(EV) * (self.L1_Bound.compute(EV) / (self.L3_Bound.compute(EV) + self.L2_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.HBM_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.Store_Bound.compute(EV))) * (self.Split_Loads.compute(EV) / (self.Store_Fwd_Blk.compute(EV) + self.Split_Loads.compute(EV) + self.DTLB_Load.compute(EV) + self.FB_Full.compute(EV) + self.L1_Latency_Dependency.compute(EV) + self.Lock_Latency.compute(EV)))) + (self.Memory_Bound.compute(EV) * (self.Store_Bound.compute(EV) / (self.L3_Bound.compute(EV) + self.L2_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.HBM_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.Store_Bound.compute(EV))) * (self.Split_Stores.compute(EV) / (self.False_Sharing.compute(EV) + self.DTLB_Store.compute(EV) + self.Store_Latency.compute(EV) + self.Split_Stores.compute(EV) + self.Streaming_Stores.compute(EV)))) + (self.Memory_Bound.compute(EV) * (self.Store_Bound.compute(EV) / (self.L3_Bound.compute(EV) + self.L2_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.HBM_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.Store_Bound.compute(EV))) * (self.Store_Latency.compute(EV) / (self.False_Sharing.compute(EV) + self.DTLB_Store.compute(EV) + self.Store_Latency.compute(EV) + self.Split_Stores.compute(EV) + self.Streaming_Stores.compute(EV)))))
    self.thresh = (val > 20)
    return val

# Total pipeline cost of Memory Address Translation related bottlenecks (data-side TLBs)
def Memory_Data_TLBs(self, EV, level):
    val = 100 *(self.Memory_Bound.compute(EV) * (self.L1_Bound.compute(EV) / max(self.Memory_Bound.compute(EV) , (self.L3_Bound.compute(EV) + self.L2_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.HBM_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.Store_Bound.compute(EV)))) * (self.DTLB_Load.compute(EV) / max(self.L1_Bound.compute(EV) , (self.Store_Fwd_Blk.compute(EV) + self.Split_Loads.compute(EV) + self.DTLB_Load.compute(EV) + self.FB_Full.compute(EV) + self.L1_Latency_Dependency.compute(EV) + self.Lock_Latency.compute(EV)))) + (self.Memory_Bound.compute(EV) * (self.Store_Bound.compute(EV) / (self.L3_Bound.compute(EV) + self.L2_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.HBM_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.Store_Bound.compute(EV))) * (self.DTLB_Store.compute(EV) / (self.False_Sharing.compute(EV) + self.DTLB_Store.compute(EV) + self.Store_Latency.compute(EV) + self.Split_Stores.compute(EV) + self.Streaming_Stores.compute(EV)))))
    self.thresh = (val > 20)
    return val
...
def Memory_Synchronization(self, EV, level):
    val = 100 *(self.Memory_Bound.compute(EV) * ((self.L3_Bound.compute(EV) / (self.L3_Bound.compute(EV) + self.L2_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.HBM_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.Store_Bound.compute(EV))) * (self.Contested_Accesses.compute(EV) + self.Data_Sharing.compute(EV)) / (self.L3_Hit_Latency.compute(EV) + self.Contested_Accesses.compute(EV) + self.SQ_Full.compute(EV) + self.Data_Sharing.compute(EV)) + (self.Store_Bound.compute(EV) / (self.L3_Bound.compute(EV) + self.L2_Bound.compute(EV) + self.L1_Bound.compute(EV) + self.HBM_Bound.compute(EV) + self.DRAM_Bound.compute(EV) + self.Store_Bound.compute(EV))) * self.False_Sharing.compute(EV) / ((self.False_Sharing.compute(EV) + self.DTLB_Store.compute(EV) + self.Store_Latency.compute(EV) + self.Split_Stores.compute(EV) + self.Streaming_Stores.compute(EV)) - self.Store_Latency.compute(EV))) + self.Machine_Clears.compute(EV) * (1 - self.Other_Nukes.compute(EV) / (self.Other_Nukes.compute(EV))))
    self.thresh = (val > 10)
    return val
...
def DSB_Misses(self, EV, level):
    val = 100 *(self.Fetch_Latency.compute(EV) * self.DSB_Switches.compute(EV) / (self.ICache_Misses.compute(EV) + self.MS_Switches.compute(EV) + self.DSB_Switches.compute(EV) + self.LCP.compute(EV) + self.Branch_Resteers.compute(EV) + self.ITLB_Misses.compute(EV)) + self.Fetch_Bandwidth.compute(EV) * self.MITE.compute(EV) / (self.MS.compute(EV) + self.DSB.compute(EV) + self.MITE.compute(EV)))
    self.thresh = (val > 10)
    return val

# Total pipeline cost of DSB (uop cache) hits - subset of the Instruction_Fetch_BW Bottleneck.
def DSB_Bandwidth(self, EV, level):
    val = 100 *(self.Frontend_Bound.compute(EV) * (self.Fetch_Bandwidth.compute(EV) / (self.Fetch_Latency.compute(EV) + self.Fetch_Bandwidth.compute(EV))) * (self.DSB.compute(EV) / (self.MS.compute(EV) + self.DSB.compute(EV) + self.MITE.compute(EV))))
    self.thresh = (val > 10)
    return val
...
def IC_Misses(self, EV, level):
    val = 100 *(self.Fetch_Latency.compute(EV) * self.ICache_Misses.compute(EV) / (self.ICache_Misses.compute(EV) + self.MS_Switches.compute(EV) + self.DSB_Switches.compute(EV) + self.LCP.compute(EV) + self.Branch_Resteers.compute(EV) + self.ITLB_Misses.compute(EV)))
    self.thresh = (val > 5)
    return val
...
def Offcore_Read_HBM_PKI(self, EV, level):
    ...
    return Data_L2_MLP(self, EV, level) if Memory > 2 else 1000 * EV("OCR.DEMAND_DATA_RD.PMM", level) / Instructions(self, EV, level)
...
def R2C_HBM_BW(self, EV, level):
    EV("OCR.DEMAND_DATA_RD.PMM", level)
    return DRAM_BW_Use(self, EV, level) if Memory > 2 else 64 * EV("OCR.DEMAND_DATA_RD.PMM", level) / 1e9 / Time(self, EV, level)
...
def CXL_Mem_Read_BW(self, EV, level):
...
def CXL_Mem_Write_BW(self, EV, level):
    return ((64 * EV("UNC_M_PMM_WPQ_INSERTS", level) / OneBillion) / Time(self, EV, level)) if PMM_App_Direct else 0

# Fraction of Uncore cycles where requests got rejected due to duplicate address already in IRQ ingress queue in the cache homing agent
def MEM_IRQ_Duplicate_Address(self, EV, level):
    val = EV("UNC_CHA_RxC_IRQ1_REJECT.PA_MATCH", level) / EV("UNC_CHA_CLOCKTICKS", level)
    self.thresh = (val > 0.1)
    return val
...
class Code_L2_Hit:
    ...
    def compute(self, EV):
        try:
            self.val = max(0 , self.ICache_Misses.compute(EV) - self.Code_L2_Miss.compute(EV))
            self.thresh = (self.val > 0.05) and self.parent.thresh
        except ZeroDivisionError:
...
class Code_STLB_Hit:
    ...
    def compute(self, EV):
        try:
            self.val = max(0 , self.ITLB_Misses.compute(EV) - self.Code_STLB_Miss.compute(EV))
            self.thresh = (self.val > 0.05) and self.parent.thresh
        except ZeroDivisionError:
...
class Code_STLB_Miss_4K:
    ...
    def compute(self, EV):
        try:
            self.val = self.Code_STLB_Miss.compute(EV) * EV("ITLB_MISSES.WALK_COMPLETED_4K", 5) / (EV("ITLB_MISSES.WALK_COMPLETED_4K", 5) + EV("ITLB_MISSES.WALK_COMPLETED_2M_4M", 5))
            self.thresh = (self.val > 0.05) and self.parent.thresh
        except ZeroDivisionError:
...
class Code_STLB_Miss_2M:
    ...
    def compute(self, EV):
        try:
            self.val = self.Code_STLB_Miss.compute(EV) * EV("ITLB_MISSES.WALK_COMPLETED_2M_4M", 5) / (EV("ITLB_MISSES.WALK_COMPLETED_4K", 5) + EV("ITLB_MISSES.WALK_COMPLETED_2M_4M", 5))
            self.thresh = (self.val > 0.05) and self.parent.thresh
        except ZeroDivisionError:
...
class MS:
    ...
    def compute(self, EV):
        try:
            self.val = max(EV("IDQ.MS_CYCLES_ANY", 3) , EV("UOPS_RETIRED.MS:c1", 3) / Retire_Fraction(self, EV, 3)) / CORE_CLKS(self, EV, 3) / 2.4
            self.thresh = (self.val > 0.05) and self.parent.thresh
        except ZeroDivisionError:
...
class L2_Hit_Latency:
    ...
    def compute(self, EV):
        try:
            self.val = Mem_L2_Hit_Cost(self, EV, 4) * EV("MEM_LOAD_RETIRED.L2_HIT", 4) * FB_Factor(self, EV, 4) / CLKS(self, EV, 4)
            self.thresh = (self.val > 0.05) and self.parent.thresh
        except ZeroDivisionError:
...
class HBM_Bound:
    ...
    errcount = 0
    sibling = None
    metricgroup = frozenset(['BvMB', 'BvML', 'MemoryBound', 'Offcore', 'Server', 'TmaL3mem'])
    maxval = None
    def compute(self, EV):
        try:
            self.val = MEM_Bound_Ratio(self, EV, 3) * EV("OCR.DEMAND_DATA_RD.PMM", 3) / EV("OCR.READS_TO_CORE.L3_MISS", 3)
            self.thresh = (self.val > 0.1) and self.parent.thresh
        except ZeroDivisionError:
            handle_error(self, "HBM_Bound zero division")
        return self.val
    desc = """
This metric estimates how often the CPU was stalled due to
High Bandwidth Memory (HBM) accesses by loads."""
...
class DRAM_Bound:
    ...
    def compute(self, EV):
        try:
            self.val = MEM_Bound_Ratio(self, EV, 3) - self.HBM_Bound.compute(EV)
            self.thresh = (self.val > 0.1) and self.parent.thresh
        except ZeroDivisionError:
...
class False_Sharing:
    ...
    def compute(self, EV):
        try:
            self.val = (Mem_Remote_HitM_Cost(self, EV, 4) * EV("OCR.DEMAND_RFO.L3_MISS:ocr_msr_val=0x103b800002", 4) + Mem_XSNP_HitM_Cost(self, EV, 4) * EV("OCR.DEMAND_RFO.L3_HIT.SNOOP_HITM", 4)) / CLKS(self, EV, 4) if DS else Mem_XSNP_HitM_Cost(self, EV, 4) * EV("OCR.DEMAND_RFO.L3_HIT.SNOOP_HITM", 4) / CLKS(self, EV, 4)
            EV("OCR.DEMAND_RFO.L3_MISS:ocr_msr_val=0x103b800002", 4)
            EV("OCR.DEMAND_RFO.L3_HIT.SNOOP_HITM", 4)
            self.thresh = (self.val > 0.05) and self.parent.thresh
        except ZeroDivisionError:
...
class Ports_Utilized_0:
    ...
    def compute(self, EV):
        try:
            self.val = (EV("EXE_ACTIVITY.EXE_BOUND_0_PORTS", 4) + max(EV("RS.EMPTY_RESOURCE", 4) - EV("RESOURCE_STALLS.SCOREBOARD", 4) , 0)) / CLKS(self, EV, 4) * (EV("CYCLE_ACTIVITY.STALLS_TOTAL", 4) - EV("EXE_ACTIVITY.BOUND_ON_LOADS", 4)) / CLKS(self, EV, 4)
            self.thresh = (self.val > 0.2) and self.parent.thresh
        except ZeroDivisionError:
...
class Metric_Offcore_Read_HBM_PKI:
    name = "Offcore_Read_HBM_PKI"
    domain = "Metric"
    maxval = 0
    errcount = 0
    area = "Info.Memory.Mix"
    metricgroup = frozenset(['Offcore', 'Server'])
    sibling = None

    def compute(self, EV):
        try:
            self.val = Offcore_Read_HBM_PKI(self, EV, 0)
            self.thresh = True
        except ZeroDivisionError:
            handle_error_metric(self, "Offcore_Read_HBM_PKI zero division")
    desc = """
High-Bandwidth Memory (HBM) accesses per kilo instruction
for reads-to-core requests (speculative; including in-core
HW prefetches)"""
...
class Metric_R2C_HBM_BW:
    name = "R2C_HBM_BW"
    domain = "GB/sec"
    maxval = 0
    errcount = 0
    area = "Info.Memory.SoC"
    metricgroup = frozenset(['HPC', 'Mem', 'MemoryBW', 'Server', 'Offcore'])
    sibling = None

    def compute(self, EV):
        try:
            self.val = R2C_HBM_BW(self, EV, 0)
            self.thresh = True
        except ZeroDivisionError:
            handle_error_metric(self, "R2C_HBM_BW zero division")
    desc = """
Average HBM BW for Reads-to-Core. See R2C_Offcore_BW."""
...
class Metric_CXL_Mem_Read_BW:
    name = "CXL_Mem_Read_BW"
    domain = "GB/sec"
    maxval = 0
    errcount = 0
    area = "Info.System"
    ...
    def compute(self, EV):
        try:
            self.val = CXL_Mem_Read_BW(self, EV, 0)
            self.thresh = True
        except ZeroDivisionError:
            handle_error_metric(self, "CXL_Mem_Read_BW zero division")
    desc = """
Average 3DXP Memory Bandwidth Use for reads [GB / sec]"""


class Metric_CXL_Mem_Write_BW:
    name = "CXL_Mem_Write_BW"
    domain = "GB/sec"
    maxval = 0
    errcount = 0
    area = "Info.System"
    ...
    def compute(self, EV):
        try:
            self.val = CXL_Mem_Write_BW(self, EV, 0)
            self.thresh = True
        except ZeroDivisionError:
            handle_error_metric(self, "CXL_Mem_Write_BW zero division")
    desc = """
Average 3DXP Memory Bandwidth Use for Writes [GB / sec]"""


class Metric_MEM_IRQ_Duplicate_Address:
    name = "MEM_IRQ_Duplicate_Address"
    domain = "System_Metric"
    maxval = 0
    errcount = 0
    area = "Info.System"
    metricgroup = frozenset(['LockCont', 'MemOffcore', 'SoC', 'Server'])
    sibling = None

    def compute(self, EV):
        try:
            self.val = MEM_IRQ_Duplicate_Address(self, EV, 0)
            self.thresh = (self.val > 0.1)
        except ZeroDivisionError:
            handle_error_metric(self, "MEM_IRQ_Duplicate_Address zero division")
    desc = """
Fraction of Uncore cycles where requests got rejected due to
duplicate address already in IRQ ingress queue in the cache
homing agent"""
...
class Setup:
    def __init__(self, r):
        ...
        n = HBM_Bound() ; r.run(n) ; o["HBM_Bound"] = n
        ...
        o["HBM_Bound"].parent = o["Memory_Bound"]
        ...
        n = Metric_Offcore_Read_HBM_PKI() ; r.metric(n) ; o["Offcore_Read_HBM_PKI"] = n
        ...
        n = Metric_R2C_HBM_BW() ; r.metric(n) ; o["R2C_HBM_BW"] = n
        n = Metric_MEM_DRAM_Read_Latency() ; r.metric(n) ; o["MEM_DRAM_Read_Latency"] = n
        n = Metric_CXL_Mem_Read_BW() ; r.metric(n) ; o["CXL_Mem_Read_BW"] = n
        n = Metric_CXL_Mem_Write_BW() ; r.metric(n) ; o["CXL_Mem_Write_BW"] = n
        n = Metric_MEM_IRQ_Duplicate_Address() ; r.metric(n) ; o["MEM_IRQ_Duplicate_Address"] = n
        ...
        o["Code_L2_Hit"].ICache_Misses = o["ICache_Misses"]
        o["Code_L2_Hit"].Code_L2_Miss = o["Code_L2_Miss"]
        o["Code_STLB_Hit"].ITLB_Misses = o["ITLB_Misses"]
        ...
        o["DRAM_Bound"].HBM_Bound = o["HBM_Bound"]
        ...
        o["Data_Cache_Memory_Bandwidth"].HBM_Bound = o["HBM_Bound"]
        ...
        o["Data_Cache_Memory_Latency"].HBM_Bound = o["HBM_Bound"]
        ...
        o["Memory_Data_TLBs"].HBM_Bound = o["HBM_Bound"]
        ...
        o["Memory_Synchronization"].HBM_Bound = o["HBM_Bound"]
        ...
        o["Other_Bottlenecks"].HBM_Bound = o["HBM_Bound"]
        ...
        o["Memory_Synchronization"].sibling = (o["Machine_Clears"], o["Contested_Accesses"], o["Data_Sharing"], o["Remote_Cache"], o["False_Sharing"],)
...
# missing nodes: Power MEM_Read_Latency MEM_Parallel_Reads
